--EXEC PRC_FTSQUOTATIONDETAILS_REPORT '2022-01-01','2022-04-30','EMS0000070','11984_1645165832011',378
--EXEC PRC_FTSQUOTATIONDETAILS_REPORT '2022-01-01','2022-04-30','','',378

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSQUOTATIONDETAILS_REPORT]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSQUOTATIONDETAILS_REPORT] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSQUOTATIONDETAILS_REPORT]
(
...
AS
/****************************************************************************************************************************************************************************
Written by : Debashis Talukder ON 28/06/2022
Module	   : Quotation Details.Refer: 0024989
1.0		Debashis		v2.0.38		23-01-2023		Multiple Contact person name to be shown in the Quotation report.Refer: 0025584
2.0		Sanchita		V2.0.38		24-01-2023		For comma seperated Email Id, Contact Person and Contact No, a space added after comma 
...
BEGIN
	...
	IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID=OBJECT_ID(N'FTSQUOTATIONDETAILS_REPORT') AND TYPE IN (N'U'))
		BEGIN
			CREATE TABLE FTSQUOTATIONDETAILS_REPORT
			(
			  USERID INT,
			  SEQ BIGINT,
			  BRANCH_ID BIGINT,
			  ...
			  RATE_SQFT DECIMAL(18,2) NULL,
			  SALESPERSON NVARCHAR(300) NULL,
			  PROJECT_NAME NVARCHAR(600) NULL
			)
			CREATE NONCLUSTERED INDEX IX1 ON FTSQUOTATIONDETAILS_REPORT (SEQ)
		END
	DELETE FROM FTSQUOTATIONDETAILS_REPORT WHERE USERID=@USERID

	SET @SqlStr=''
	SET @SqlStr='INSERT INTO FTSQUOTATIONDETAILS_REPORT(USERID,SEQ,BRANCH_ID,BRANCHDESC,EMPUSERID,EMPCODE,EMPID,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,DATEOFJOINING,LOGINID,REPORTTOID,REPORTTOUID,'
	SET @SqlStr+='REPORTTO,RPTTODESG,QUOTATIONNO,QUOTATIONDATE,QUOTATIONDATEORDBY,SHOP_CODE,CUSTNAME,CUSTADDRESS,CUSTEMAIL,CONTACTPERSON,CONTACTNO,PROD_ID,PRODUCT_NAME,RATE_SQFT,SALESPERSON,'
	SET @SqlStr+='PROJECT_NAME) '
	SET @SqlStr+='SELECT '+LTRIM(RTRIM(STR(@USERID)))+' AS USERID,ROW_NUMBER() OVER(ORDER BY CONVERT(NVARCHAR(10),QH.QUOTATIONSAVE_DATE,120)) AS SEQ,BR.BRANCH_ID,BR.BRANCH_DESCRIPTION,USR.USER_ID AS EMPUSERID,CNT.cnt_internalId AS EMPCODE,'
	...
	SET @SqlStr+='USR.user_loginId AS LOGINID,RPTTO.REPORTTOID,RPTTO.REPORTTOUID,RPTTO.REPORTTO,RPTTO.RPTTODESG,QH.QUOTATION_NUMBER,CONVERT(NVARCHAR(10),QH.QUOTATIONSAVE_DATE,105) AS QUOTATIONDATE,'
	--Rev 1.0
	...
	-- Rev 2.0 (SELECT '','' changed to SELECT '', '' )
	...
	SET @SqlStr+='WHERE MQC.USER_ID=USR.USER_ID AND MQC.USER_ID=QH.USER_ID AND QH.QUOTATION_NUMBER=MQC.QUOTATION_NUMBER FOR XML PATH('''')), 1, 1, ''''),'''')) AS CUSTEMAIL,'
	...
	SET @SqlStr+='WHERE MQC.USER_ID=USR.USER_ID AND MQC.USER_ID=QH.USER_ID AND QH.QUOTATION_NUMBER=MQC.QUOTATION_NUMBER FOR XML PATH('''')), 1, 1, ''''),'''')) AS CONTACTPERSON,'
	...
	SET @SqlStr+='WHERE MQC.USER_ID=USR.USER_ID AND MQC.USER_ID=QH.USER_ID AND QH.QUOTATION_NUMBER=MQC.QUOTATION_NUMBER FOR XML PATH('''')), 1, 1, ''''),'''')) AS CONTACTNO,'
	SET @SqlStr+='QD.PROD_ID,QD.PRODUCT_NAME,QD.RATE_SQFT,MUSR.user_name AS SALESPERSON,QH.PROJECT_NAME '
	--End of Rev 1.0
	...
	SET @SqlStr+='INNER JOIN FSMAPIQUOTATIONDETAILS QD ON QH.QUOTATION_NUMBER=QD.QUOTATION_NUMBER '
