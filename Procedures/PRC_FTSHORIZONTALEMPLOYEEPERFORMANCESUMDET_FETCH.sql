...
BEGIN
	...
	IF @REPORTTYPE='Summary'
		BEGIN
			SET @Strsql='SELECT ROW_NUMBER() OVER(ORDER BY EMPNAME,LOGIN_DATETIME) AS SEQ,User_Id AS USERID,LOGIN_DATETIME,LOGINDATE,CONTACTNO,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,'
			...
			SET @Strsql+='ISNULL(SHOPACT.NEWSHOP_VISITED,0) AS NEWSHOP_VISITED,ISNULL(SHOPACT.RE_VISITED,0) AS RE_VISITED,SHPTYPE.SHOPTYPEID,SHPTYPE.SHOPTYPENAME,SHPTYPE.SHOPVISITED '
	...
	ELSE IF @REPORTTYPE='Details'
		BEGIN
			SET @Strsql='SELECT MU.USER_ID,CNT.CNT_INTERNALID,MS.Shop_Name AS CUSTNAME,MS.ADDRESS,MS.Shop_Owner_Contact AS MOBILENO,ST.NAME AS SHOPTYPE,CONVERT(NVARCHAR(10),SHOPACT.visited_time,105) AS VISITDATE,'
			SET @Strsql+='CONVERT(NVARCHAR(10),SHOPACT.visited_time,108) AS VISITTIME,SHOPACT.distance_travelled AS SPENTDURATION '
			SET @Strsql+='FROM tbl_shoptype ST '
			SET @Strsql+='INNER JOIN tbl_Master_shop MS ON ST.TypeId=MS.type '
			SET @Strsql+='INNER JOIN tbl_master_user MU ON MS.Shop_CreateUser=MU.USER_ID AND MU.user_inactive=''N'' '
			SET @Strsql+='INNER JOIN #TEMPCONTACT CNT ON MU.user_contactId=CNT.cnt_internalId '
