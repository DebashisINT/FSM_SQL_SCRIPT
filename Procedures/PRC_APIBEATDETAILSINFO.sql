...
BEGIN
	...
	IF @ACTION='BEATDETLIST'
		BEGIN
			SELECT ATTEN.USER_ID,BH.ID AS beat_id,BH.NAME AS beat_name FROM FSM_GROUPBEAT BH
			INNER JOIN tbl_fts_UserAttendanceLoginlogout ATTEN ON BH.ID=ATTEN.BEAT_ID
	...
	IF @ACTION='UPDATEBEAT'
		BEGIN
			IF EXISTS(SELECT USER_ID FROM tbl_fts_UserAttendanceLoginlogout WHERE USER_ID=@USER_ID AND CONVERT(NVARCHAR(10),Work_datetime,120)=CONVERT(NVARCHAR(10),@BEAT_DATE,120))
				BEGIN
					UPDATE tbl_fts_UserAttendanceLoginlogout SET Beat_ID=@BEAT_ID
					...
					SELECT ATTEN.USER_ID,BH.ID AS updated_beat_id,BH.NAME AS beat_name FROM FSM_GROUPBEAT BH
					INNER JOIN tbl_fts_UserAttendanceLoginlogout ATTEN ON BH.ID=ATTEN.BEAT_ID
