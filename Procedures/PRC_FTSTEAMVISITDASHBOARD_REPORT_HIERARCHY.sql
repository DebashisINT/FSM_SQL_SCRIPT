--EXEC PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY '2022-05-24','5','','','ALL','Summary',136,108390
--EXEC PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY '2022-02-09','','','','EMP','Detail',53028
--EXEC PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY '2022-02-09','','','','ALL','Summary',53028
--EXEC PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY '2022-02-09','','','','AT_WORK','Detail',53028
--EXEC PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY '2022-02-09','','','','AT_WORK','Summary',53028
--EXEC PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY '2022-02-09','15','','EMP0000002','AT_WORKTRAVEL','Summary',53028
--EXEC PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY '2022-02-09','','','','ON_LEAVE','Detail',53028
--EXEC PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY '2022-02-09','15','','','NOT_LOGIN','Detail',53028
--EXEC PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY '2022-02-09','','','','GRAPH','Detail',53028

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY]
(
...
11.0	V2.0.36		Sanchita	10-11-2022		When an Employee is mapped with Multiple States and Multiple Branch, the count of that employee should be considered as One
												Refer: 25441
12.0	V2.0.37		Sanchita	01-12-2022		FSM Dashboard : NEW tab shall be implemented "Team Visit - Hierarchy & Channel Wise" after the 'Team Visit' Tab
												Refer: 25468
												NEW SP WRITTEN
13.0	v2.0.38		Sanchita	04-01-2023		Dashboard showing wrong data when clicked on Employee Strength in Visit tab. refer: 25567
14.0	V2.0.40		Sanchita	05-04-2023		ITC user dashboard is taking too much time to respond for AE & AM users login credentials.
...
BEGIN
	...
	IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID=OBJECT_ID(N'FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY') AND TYPE IN (N'U'))
		BEGIN
			CREATE TABLE FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY
			(
			  USERID INT,
			  ACTION NVARCHAR(20),
			  RPTTYPE NVARCHAR(20),
			...
			CREATE NONCLUSTERED INDEX IX1 ON FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY (SEQ)
		END
	DELETE FROM FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY WHERE USERID=@USERID

	-- Rev 14.0 [ In the full query below all FTS_EmployeeBranchMap have been replaced by #TMPEmployeeBranchMap
	SET @Strsql=''
	IF @ACTION='ALL' AND @RPTTYPE='Summary'
		BEGIN
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,EMPCNT) '
			-- Rev 11.0
			...
			SET @Strsql+='SELECT '+LTRIM(RTRIM(STR(@USERID)))+' AS USERID,''EMP'' AS ACTION,''Summary'' AS RPTTYPE,COUNT(cnt_internalId) AS EMPCNT FROM ( '
			SET @Strsql+='SELECT '+LTRIM(RTRIM(STR(@USERID)))+' AS USERID,''EMP'' AS ACTION,''Summary'' AS RPTTYPE,CNT.cnt_internalId FROM tbl_master_employee EMP WITH (NOLOCK) '
			-- End of Rev 11.0
			...
			IF(@ActivateEmployeeBranchHierarchy = 0)
				BEGIN
					-- Rev 9.0
					--IF @STATEID<>'' 
					--	SET @Strsql+='WHERE EXISTS (SELECT State_Id FROM #STATEID_LIST AS STA WHERE STA.State_Id=ST.id) and EXISTS (SELECT Branch_Id from #BRANCHID_LIST AS BR WHERE EXISTS(SELECT BMAP.BranchId FROM #TMPEmployeeBranchMap BMAP WHERE BR.Branch_Id=BMAP.BranchId) ) '
					--IF @STATEID=''
					--	SET @Strsql+='WHERE EXISTS (SELECT Branch_Id from #BRANCHID_LIST AS BR WHERE EXISTS(SELECT BMAP.BranchId FROM #TMPEmployeeBranchMap BMAP WHERE BR.Branch_Id=BMAP.BranchId) ) '
					IF @STATEID<>'' 
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,AT_WORK) '
			-- Rev 11.0
			...
			IF(@ActivateEmployeeBranchHierarchy = 0)
				BEGIN
					-- Rev 9.0
					--SET @Strsql+='and EXISTS (SELECT Branch_Id from #BRANCHID_LIST AS BR WHERE EXISTS(SELECT BMAP.BranchId FROM #TMPEmployeeBranchMap BMAP WHERE BR.Branch_Id=BMAP.BranchId) ) '
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,ON_LEAVE) '
			-- Rev 11.0
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,NOT_LOGIN) '
	...
	ELSE IF @ACTION='AT_WORK' AND @RPTTYPE='Summary'
		BEGIN
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,STATEID,STATE,DESIGNATION,EMPCNT) '
	...
	ELSE IF @ACTION='EMP' AND @RPTTYPE='Detail'
		BEGIN
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCH_ID,BRANCHDESC,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,CONTACTNO,REPORTTOUID,REPORTTO,'
	...
	ELSE IF @ACTION='AT_WORK' AND @RPTTYPE='Detail'
		BEGIN
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCH_ID,BRANCHDESC,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,CONTACTNO,REPORTTOUID,REPORTTO,LOGGEDIN,'
	...
	ELSE IF @ACTION='AT_WORKTRAVEL' AND @RPTTYPE='Summary'
		BEGIN
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCH_ID,BRANCHDESC,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,CONTACTNO,REPORTTOUID,REPORTTO,DISTANCE_COVERED) '
	...
	ELSE IF @ACTION='ON_LEAVE' AND @RPTTYPE='Detail'
		BEGIN
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCH_ID,BRANCHDESC,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,REPORTTOUID,REPORTTO,LEAVEDATE,DEPARTMENT) '
	...
	ELSE IF @ACTION='NOT_LOGIN' AND @RPTTYPE='Detail'
		BEGIN
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCH_ID,BRANCHDESC,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,CONTACTNO,REPORTTOUID,REPORTTO,DEPARTMENT) '
	...
	ELSE IF @ACTION='GRAPH' AND @RPTTYPE='Detail'
		BEGIN
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCH_ID,BRANCHDESC,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,REPORTTOUID,REPORTTO,LOGGEDIN,'
			...
			--Rev 10.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSTEAMVISITDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCH_ID,BRANCHDESC,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,REPORTTOUID,REPORTTO,LEAVEDATE) '
			SET @Strsql+='SELECT '+LTRIM(RTRIM(STR(@USERID)))+' AS USERID,''ON_LEAVE'' AS ACTION,''Detail'' AS RPTTYPE,ROW_NUMBER() OVER(ORDER BY DESIGNATION) AS SEQ,BRANCH_ID,BRANCHDESC,EMPID,EMPCODE,EMPNAME,'
