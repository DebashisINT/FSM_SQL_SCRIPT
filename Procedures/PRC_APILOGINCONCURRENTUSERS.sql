...
BEGIN
	...
	IF @ACTION='INSERTUSERIMEI'
		BEGIN
			IF NOT EXISTS(SELECT IMEI_NO FROM APP_LOGIN_CONCURRENTUSERS WHERE USER_ID=@USER_ID AND IMEI_NO=@IMEI)
				BEGIN
					INSERT INTO APP_LOGIN_CONCURRENTUSERS(USER_ID,IMEI_NO,CREATEDATE)
					SELECT @USER_ID,@IMEI,@DATE_TIME

					SELECT USER_ID,IMEI_NO,CREATEDATE FROM APP_LOGIN_CONCURRENTUSERS WHERE USER_ID=@USER_ID AND IMEI_NO=@IMEI
				END
		END
	IF @ACTION='FETCHUSERIMEI'
		BEGIN
			SELECT USER_ID,IMEI_NO,CREATEDATE FROM APP_LOGIN_CONCURRENTUSERS WHERE USER_ID=@USER_ID
		END
	IF @ACTION='DELETEUSERIMEI'
		BEGIN
			IF EXISTS(SELECT USER_ID FROM APP_LOGIN_CONCURRENTUSERS WHERE USER_ID=@USER_ID)
				BEGIN
