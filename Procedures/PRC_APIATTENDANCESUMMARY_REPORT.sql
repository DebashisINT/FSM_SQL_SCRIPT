--EXEC PRC_APIATTENDANCESUMMARY_REPORT

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_APIATTENDANCESUMMARY_REPORT]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_APIATTENDANCESUMMARY_REPORT] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_APIATTENDANCESUMMARY_REPORT]
--WITH ENCRYPTION
AS
/****************************************************************************************************************************************************************************
Written by : Debashis Talukder ON 30/06/2023
Module	   : Attendance Summary Report.Refer: 0026236
...
5.0		v2.0.49		20/09/2024		Debashis	If the global settings AttendanceCalenderConsiderDayend is set as 1 then consider the below changes.
												1. It will check the Day End for Qualified attendance
												2. The same shall reflect in Attendance Calendar and in Attendance Summary reports.Refer: 0027687
****************************************************************************************************************************************************************************/
BEGIN
	SET NOCOUNT ON
	...
	--End of Rev 1.0
	--Rev 5.0
	DECLARE @AttendanceCalenderConsiderDayend NCHAR(1)='0'
	--End of Rev 5.0
	...
	--Rev 5.0
	SELECT @AttendanceCalenderConsiderDayend=(SELECT [Value] FROM FTS_APP_CONFIG_SETTINGS WHERE [Key]='AttendanceCalenderConsiderDayend')
	--End of Rev 5.0
	...
	--Rev 5.0 && A new column has been added as ISEND BIT
	...
	--Rev 5.0 && A new column has been added as ISEND
	SET @SqlStr=''
	SET @SqlStr='INSERT INTO #TMPDAYSTARTENDQA(USERID,DAYSTTIME,DAYENDTIME,cnt_internalId,STARTENDDATE,STARTENDDATEORDBY,ISEND) '
	...
	SET @SqlStr+='CONVERT(NVARCHAR(10),DAYSTEND.STARTENDDATE,105) AS STARTENDDATE,CONVERT(NVARCHAR(10),DAYSTEND.STARTENDDATE,120) AS STARTENDDATEORDBY,0 AS ISEND '
	...
	SET @SqlStr+='CONVERT(NVARCHAR(10),DAYSTEND.STARTENDDATE,105) AS STARTENDDATE,CONVERT(NVARCHAR(10),DAYSTEND.STARTENDDATE,120) AS STARTENDDATEORDBY,ISEND '
	SET @SqlStr+='FROM FSMUSERWISEDAYSTARTEND DAYSTEND WITH (NOLOCK) '
	SET @SqlStr+='INNER JOIN tbl_master_user USR WITH (NOLOCK) ON DAYSTEND.User_Id=USR.user_id AND USR.user_inactive=''N'' '
	SET @SqlStr+='INNER JOIN #TEMPCONTACT CNT ON CNT.cnt_internalId=USR.user_contactId '
	--Rev 4.0
	...
	--End of Rev 4.0
	SET @SqlStr+='AND CONVERT(NVARCHAR(10),DAYSTEND.STARTENDDATE,120) BETWEEN CONVERT(NVARCHAR(10),'''+@FROMDATE+''',120) AND CONVERT(NVARCHAR(10),'''+@TODATE+''',120) '
	SET @SqlStr+='GROUP BY DAYSTEND.User_Id,CNT.cnt_internalId,CONVERT(NVARCHAR(10),DAYSTEND.STARTENDDATE,105),CONVERT(NVARCHAR(10),DAYSTEND.STARTENDDATE,120),ISEND '
	...
	IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID=OBJECT_ID(N'APIATTENDANCESUMMARY_REPORT') AND TYPE IN (N'U'))
		BEGIN
			CREATE TABLE APIATTENDANCESUMMARY_REPORT
			(
			BRANCH_ID BIGINT,
			...
			CREATE NONCLUSTERED INDEX IX1 ON APIATTENDANCESUMMARY_REPORT (USERID)
		END
	TRUNCATE TABLE APIATTENDANCESUMMARY_REPORT

	SET @SqlStr=''
	SET @SqlStr='INSERT INTO APIATTENDANCESUMMARY_REPORT(BRANCH_ID,BRANCH_DESCRIPTION,USERID,EMPCODE,EMPID,EMPNAME,CONTACTNO,STATEID,STATE,DEG_ID,DESIGNATION,DATEOFJOINING,REPORTTOID,'
	SET @SqlStr+='REPORTTOUID,REPORTTO,RPTTODESG,DSTYPE,CH_ID,CHANNEL,TOTWORKINGDAYS,TOTALATTENDANCE,TOTALABSENT,TOTALQUALIFIEDPRESENT) '
	SET @SqlStr+='SELECT BRANCH_ID,BRANCH_DESCRIPTION,USERID,EMPCODE,EMPID,EMPNAME,CONTACTNO,STATEID,STATE,DEG_ID,DESIGNATION,DATEOFJOINING,REPORTTOID,REPORTTOUID,REPORTTO,RPTTODESG,DSTYPE,CH_ID,'
	--Rev 1.0
	--SET @SqlStr+='CHANNEL,'+STR(LTRIM(RTRIM(@days)))+' AS TOTWORKINGDAYS,SUM(ATTENDANCE) AS TOTALATTENDANCE,'+STR(LTRIM(RTRIM(@days)))+'-SUM(ATTENDANCE) AS TOTALABSENT,'
	SET @SqlStr+='CHANNEL,'+STR(LTRIM(RTRIM(@DAYCOUNT)))+' AS TOTWORKINGDAYS,SUM(ATTENDANCE) AS TOTALATTENDANCE,'+STR(LTRIM(RTRIM(@DAYCOUNT)))+'-SUM(ATTENDANCE) AS TOTALABSENT,'
	--End of Rev 1.0
	SET @SqlStr+='SUM(QUALIFIEDPRESENT) AS TOTALQUALIFIEDPRESENT FROM('
	...
	IF @AttendanceCalenderConsiderDayend='0'
		SET @SqlStr+='CASE WHEN (ISNULL(SHOPACT.NEWSHOP_VISITED,0)+ISNULL(SHOPACT.RE_VISITED,0))>=20 AND '
	ELSE IF @AttendanceCalenderConsiderDayend='1'
	...
	--Rev 5.0
	IF @AttendanceCalenderConsiderDayend='1'
		BEGIN
