...
--drop PROC [dbo].[PRC_WODTARGETASSIGN ]
...
************************************************************************************************/ 
	...
	,@WODCOUNT bigint=0,@WODNAME nvarchar(200)='',@ORDERAMOUNT Numeric(18,4)=0,@COLLECTION Numeric(18,4)=0, @ORDERQTY Numeric(18,4)=0
		...
		BEGIN
					...
					Select 	TARGETLEVELASSIGNID,TARGETLEVELASSIGNNAME,INTERNALID,TIMEFRAME ,STARTDATE,ENDDATE,WODCOUNT
					...
					BEGIN 
							...
							if NOT EXISTS (select 'Y' from FSM_WODTARGETASSIGN_DETAILS   where TARGETLEVELASSIGNID=@TARGETLEVELID and TARGETLEVELASSIGNNAME=@TARGETLEVEL and 	INTERNALID=@INTERNALID and TIMEFRAME=@TIMEFRAME
							and STARTDATE=@STARTEDATE and ENDDATE=@ENDDATE and WODCOUNT=@WODCOUNT )
							Begin
								...
								INTERNALID,TIMEFRAME,STARTDATE,ENDDATE,WODCOUNT,TARGETLEVELID,CREATEDBY,CREATEDON	
								)
		...
		BEGIN
					...
					update FSM_WODTARGETASSIGN set UPDATEDBY=@USER_ID,UPDATEDON=GETDATE() where WODTARGET_ID=@TARGET_ID
		...
		select WODTARGETDETAILS_ID,ROW_NUMBER() OVER(ORDER BY WODTARGETDETAILS_ID ASC) AS SlNO,WODTARGETDOCNUMBER TARGETDOCNUMBER,TARGETLEVELASSIGNID TARGETLEVELID,TARGETLEVELASSIGNNAME TARGETLEVEL,INTERNALID,
						TIMEFRAME,CONVERT(nvarchar(10),STARTDATE,105) STARTEDATE, CONVERT(nvarchar(10),ENDDATE,105) ENDDATE,WODCOUNT
		from FSM_WODTARGETASSIGN_DETAILS DETAILS where DETAILS.WODTARGET_ID=@TARGET_ID
		end
		else IF @ACTION = 'GETHEADERWODTARGET'
		Begin
				select WODTARGET_ID,TARGETLEVELID TARGETLEVEL,WODTARGETDOCNUMBER TARGETDOCNUMBER,WODTARGETDATE TARGETDATE,CREATEDBY,CREATEDON,UPDATEDBY,UPDATEDON 
		...
		Begin
				if EXISTS (select 'Y' from FSM_WODTARGETASSIGN_DETAILS where TARGETLEVELASSIGNNAME=@UNIQUETARGETLEVEL and TARGETLEVELASSIGNID=@UNIQUETARGETLEVELID and INTERNALID=@UNIQUEINTERNALID  
				and TIMEFRAME=@UNIQUETIMEFRAME
				and STARTDATE=@UNIQUESTARTEDATE
				and ENDDATE=@UNIQUEENDDATE and TARGETLEVELID=@TargetType)
				Begin
					set @ReturnValue=1;
				end
				Else
