--EXEC PRC_FTSQUALIFIEDATTENDANCE_REPORT_AUTOMAIL '2022-10-11','2022-11-08','1,118,119,120,121,122,123,124,125','','',378
--EXEC PRC_FTSQUALIFIEDATTENDANCE_REPORT_AUTOMAIL '2022-03-09','2022-03-09','1','EMA0000008,EMA0000016,EMA0000012,EMM0000002,EMA0000020','1,2,3,4,5,6,7,8',378

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSQUALIFIEDATTENDANCE_REPORT_AUTOMAIL]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSQUALIFIEDATTENDANCE_REPORT_AUTOMAIL] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSQUALIFIEDATTENDANCE_REPORT_AUTOMAIL]
(
...
BEGIN
	...
	EMPNAME NVARCHAR(300),CONTACTNO NVARCHAR(100),STATEID BIGINT,STATE NVARCHAR(100),DEG_ID BIGINT,DESIGNATION NVARCHAR(300),DATEOFJOINING NVARCHAR(10),REPORTTOID NVARCHAR(100),REPORTTOUID NVARCHAR(100),
	REPORTTO NVARCHAR(300),RPTTODESG NVARCHAR(100),DSTYPE NVARCHAR(300),CH_ID BIGINT,CHANNEL NVARCHAR(100),PRESENTABSENT INT)
	...
	--Rev 1.0
	INSERT INTO #TMPATTENDACEQADET EXEC [PRC_FTSQUALIFIEDATTENDANCE_FETCH_AUTOMAIL] @FROMDATE,@TODATE,@BRANCHID,@EMPID,@CHANNELID,@USERID
		...
		BEGIN
			WHILE @LOOPCOUNT<=@DAYCOUNT
				BEGIN				
					...
					--Rev 1.0
					--INSERT INTO #TMPATTENDACEQA EXEC [PRC_FTSQUALIFIEDATTENDANCE_FETCH_AUTOMAIL] @COLUMN_DATE,@TODATE,@BRANCHID,@EMPID,@CHANNELID,@USERID
					...
					INSERT INTO #TMPATTENDACEQA(STARTENDDATE,STARTENDDATEORDBY,BRANCH_ID,BRANCH_DESCRIPTION,USERID,EMPCODE,EMPID,EMPNAME,CONTACTNO,STATEID,STATE,DEG_ID,DESIGNATION,DATEOFJOINING,REPORTTOID,
					REPORTTOUID,REPORTTO,RPTTODESG,DSTYPE,CH_ID,CHANNEL,PRESENTABSENT)
					SELECT STARTENDDATE,STARTENDDATEORDBY,BRANCH_ID,BRANCH_DESCRIPTION,USERID,EMPCODE,EMPID,EMPNAME,CONTACTNO,STATEID,STATE,DEG_ID,DESIGNATION,DATEOFJOINING,REPORTTOID,
					REPORTTOUID,REPORTTO,RPTTODESG,DSTYPE,CH_ID,CHANNEL,PRESENTABSENT FROM #TMPATTENDACEQADET WHERE STARTENDDATEORDBY BETWEEN @COLUMN_DATE AND @TODATE
	...
	IF EXISTS (SELECT * FROM sys.objects WHERE object_id=OBJECT_ID(N'EMPLOYEEATTENDANCE_MAILQA') AND TYPE IN (N'U'))
		...
		set @sqlQry = '' -- 'select '
		set @sqlInsertQry = '' -- 'insert into EMPLOYEEATTENDANCE_MAILQA values ( ' 
		set @cnt = 1
		SET @sqlInsertQry2 = '' -- 'INSERT INTO EMPLOYEEATTENDANCE_MAILQA SELECT '

		-- Update header row
		declare db_cursor cursor for
		select HEADNAME, HEADSHRTNAME from #TMPEHEADINGQA where PARRENTID<>0 order by HEADID
		open db_cursor
		...
		begin
			...
			begin
				...
				begin
					...
					set @sqlInsertQry2 += 'ISNULL('+ @HEADSHRTNAME+','''') AS '+@HEADSHRTNAME
				end
				else
				begin
		...
		SET @sqlInsertQry2 = 'select ' +@sqlInsertQry2+  ' into EMPLOYEEATTENDANCE_MAILQA from #EMPLOYEEATTENDANCEQA '
