...
BEGIN    
	...
	BEGIN
		...
		if(isnull(@SearchKey,'')!='')
		Begin		
			select convert(varchar(500),EMAP.TARGET_ID)+'|'+CON.cnt_internalId as ID , 
			...
			where EMAP.TARGET_LEVEL_SHORTNAME=@LEVEL_SHORTNAME and
		...
		Begin
			select convert(varchar(500),EMAP.TARGET_ID)+'|'+CON.cnt_internalId as ID , 
				...
				INNER JOIN	tbl_master_contact CON ON EMAP.TARGET_EMPCNTID=CON.cnt_internalId
				INNER JOIN tbl_master_employee EMP ON CON.cnt_internalId=EMP.emp_contactId
			where EMAP.TARGET_LEVEL_SHORTNAME=@LEVEL_SHORTNAME 
		End
	...
	BEGIN
		...
		if(isnull(@SearchKey,'')!='')
		Begin		
			select convert(varchar(5000),sProducts_ID)+'|'+sProducts_Name as ID ,sProducts_Code as INTERNALID,sProducts_Code as CODE, sProducts_Name as NAME 
		...
		Begin
			select convert(varchar(5000),sProducts_ID)+'|'+sProducts_Code as ID ,sProducts_Code as INTERNALID,sProducts_Code as CODE, sProducts_Name as NAME 
			from Master_sProducts 
