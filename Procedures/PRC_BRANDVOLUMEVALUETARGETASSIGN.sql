...
************************************************************************************************/ 
	...
	,@BRANDID bigint=0,@BRANDNAME nvarchar(200)='',@ORDERAMOUNT Numeric(18,4)=0,@COLLECTION Numeric(18,4)=0, @ORDERQTY Numeric(18,4)=0
		...
		BEGIN
					...
					INSERT INTO FSM_BRANDTARGETASSIGN(BRANDTARGET_ID,BRANDTARGETTYPE,BRANDTARGETDOCNUMBER,BRANDTARGETDATE,CREATEDBY,CREATEDON) 
					VALUES(@LastCount+1,@TargetType,@TargetNo,@TargetDate,@USER_ID,GETDATE());			

					
					DECLARE db_cursor CURSOR FOR  
					Select 	TARGETLEVELID,TARGETLEVEL,INTERNALID,TIMEFRAME ,STARTEDATE,ENDDATE,BRANDID,BRANDNAME,ORDERAMOUNT,ORDERQTY
					...
					BEGIN 
							...
							INTERNALID,TIMEFRAME,STARTEDATE,ENDDATE,BRANDID,BRANDNAME,ORDERAMOUNT,ORDERQTY,CREATEDBY,CREATEDON	
							)
							Select @LastCountDetails+1,@LastCount+1,@TargetNo
							,@TARGETLEVELID,@TARGETLEVEL,@INTERNALID,@TIMEFRAME ,@STARTEDATE,@ENDDATE,@BRANDID,@BRANDNAME,@ORDERAMOUNT,@ORDERQTY,@USER_ID,GETDATE()
		...
		select BRANDTARGETDETAILS_ID,ROW_NUMBER() OVER(ORDER BY BRANDTARGETDETAILS_ID ASC) AS SlNO,BRANDTARGETDOCNUMBER TARGETDOCNUMBER,TARGETLEVELID,TARGETLEVEL,INTERNALID,TIMEFRAME,STARTEDATE,ENDDATE,BRANDID,BRANDNAME,ORDERAMOUNT,ORDERQTY
		from FSM_BRANDTARGETASSIGN_DETAILS DETAILS where DETAILS.BRANDTARGET_ID=@TARGET_ID
		end
		else IF @ACTION = 'GETHEADERSALESTARGET'
		Begin
				select BRANDTARGET_ID,BRANDTARGETTYPE TARGETLEVEL,BRANDTARGETDOCNUMBER TARGETDOCNUMBER,BRANDTARGETDATE TARGETDATE,CREATEDBY,CREATEDON,UPDATEDBY,UPDATEDON 
