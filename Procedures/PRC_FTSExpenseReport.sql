...
Begin
	...
	BEGIN
		...
		IF(ISNULL(@expid_list,'')<>'')
			...
			ELSE
				BEGIN
					INSERT INTO #TEMP_EXPENSETYPE 
					SELECT EXP_ID FROM Master_ExpenseType
				END
		...
		SET @STR=@STR+ 'REPORTTO_NAME, EXPENSE_TYPE, USERID) '

		SET @STR=@STR+ 'SELECT VISITED_DATE, city_name, EMPID,EMP_NAME,DESIGNATION,SUPERVISOR_NAME,'
		SET @STR=@STR+ '(CASE WHEN MAX(StationCode)=0 THEN ''In Station'' WHEN  MAX(StationCode)=1 THEN ''Ex Station'' ELSE ''Out Station'' END ) EXPENSE_TYPE, '
		SET @STR=@STR+ 'USERID FROM ( '
		...
		SET @STR=@STR+ 'SA.StationCode , U.user_id AS USERID FROM tbl_trans_shopActivitysubmit SA '
		...
		SET @STR=@STR+ 'INNER JOIN Master_ExpenseType EXPT ON SA.StationCode=EXPT.EXP_ID '
		SET @STR=@STR+ 'AND EXISTS (SELECT 1 FROM #TEMP_EXPENSETYPE ET WHERE ET.expid=EXPT.EXP_ID) '
		...
		SET @STR=@STR+ 'GROUP BY SHOP_ACT.VISITED_DATE, city_name, EMPID,EMP_NAME,DESIGNATION,SUPERVISOR_NAME,USERID '
		EXEC (@STR)
		...
		SET @STR=@STR+ 'SELECT ROW_NUMBER() OVER (ORDER BY EMPID) SL, REIMBURSEMENT_DATE, HQ_NAME, EMPID,EMP_NAME,EMP_DESIGNATION,REPORTTO_NAME,EXPENSE_TYPE,'
		SET @STR=@STR+ '0 AS OTHER_ALLOWANCE, ISNULL(RA.AMOUNT,0) AS DAILY_ALLOWANCE,  '
		SET @STR=@STR+ '(ISNULL(RA.AMOUNT,0)+0) TOTAL_ALLOWANCE, '+LTRIM(RTRIM(STR(@USER_ID)))+' AS USER_ID, '
		SET @STR=@STR+ '(case when IMG.IMG_CNT >0 then 1 else 0 end) AS IS_IMAGE '
		SET @STR=@STR+ 'FROM #TEMP_shopActivitysubmit SHOP_ACT '
		SET @STR+='LEFT OUTER JOIN(SELECT UserID,SUM(ISNULL(AMOUNT,0)) AS AMOUNT,CAST(DATE AS DATE) AS DATE FROM FTS_Reimbursement_Application '
		SET @STR+='WHERE Expence_type=''Allowance'' '
		SET @STR+='GROUP BY UserID,CAST(DATE AS DATE)) RA ON SHOP_ACT.USERID=RA.UserID AND CONVERT(DATE,SHOP_ACT.REIMBURSEMENT_DATE)=CONVERT(DATE,RA.DATE) '
	...
	BEGIN
		SELECT cast(EXP_ID as varchar(50)) as expid, EXPENSE_TYPE as expense_type FROM Master_ExpenseType
			order by expid
	END
	if (@ACTION='LOADIMAGE')
	BEGIN
