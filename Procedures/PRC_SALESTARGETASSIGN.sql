...
************************************************************************************************/ 
	...
	,@NEWVISIT bigint=0,@REVISIT bigint=0
		...
		BEGIN
					...
					INSERT INTO FSM_SALESTARGETASSIGN(SALESTARGET_ID,TARGETLEVEL,TARGETDOCNUMBER,TARGETDATE,CREATEDBY,CREATEDON) 
					VALUES(@LastCount+1,@TargetType,@TargetNo,@TargetDate,@USER_ID,GETDATE());			

					
					DECLARE db_cursor CURSOR FOR  
					Select 	TARGETLEVELID,TARGETLEVEL,INTERNALID,TIMEFRAME ,STARTEDATE,ENDDATE,NEWVISIT,REVISIT,ORDERAMOUNT,COLLECTION,ORDERQTY
					...
					BEGIN 
							...
							INTERNALID,TIMEFRAME,STARTEDATE,ENDDATE,NEWVISIT,REVISIT,ORDERAMOUNT,COLLECTION,ORDERQTY,CREATEDBY,CREATEDON	
							)
							Select @LastCountDetails+1,@LastCount+1,@TargetNo
							,@TARGETLEVELID,@TARGETLEVEL,@INTERNALID,@TIMEFRAME ,@STARTEDATE,@ENDDATE,@NEWVISIT,@REVISIT,@ORDERAMOUNT,@COLLECTION,@ORDERQTY,@USER_ID,GETDATE()
		...
		select SALESTARGETDETAILS_ID,ROW_NUMBER() OVER(ORDER BY SALESTARGETDETAILS_ID ASC) AS SlNO,TARGETDOCNUMBER,TARGETLEVELID,TARGETLEVEL,INTERNALID,TIMEFRAME,STARTEDATE,ENDDATE,NEWVISIT,REVISIT,ORDERAMOUNT,COLLECTION,ORDERQTY
