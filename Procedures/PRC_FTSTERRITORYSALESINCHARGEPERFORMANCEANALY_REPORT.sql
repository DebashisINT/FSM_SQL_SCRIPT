--EXEC PRC_FTSTERRITORYSALESINCHARGEPERFORMANCEANALY_REPORT '2022-11-25','2022-11-25','','',378

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSTERRITORYSALESINCHARGEPERFORMANCEANALY_REPORT]') AND TYPE in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSTERRITORYSALESINCHARGEPERFORMANCEANALY_REPORT] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSTERRITORYSALESINCHARGEPERFORMANCEANALY_REPORT]
(
...
BEGIN
	...
	DECLARE @Strsql NVARCHAR(MAX),@SqlStrTable NVARCHAR(MAX),@FIRSTDATEOFMONTH DATETIME,@CURRENTDATEOFMONTH DATETIME
	...
	IF OBJECT_ID('tempdb..#TMPATTENLOGINTSIA') IS NOT NULL
		DROP TABLE #TMPATTENLOGINTSIA
	CREATE TABLE #TMPATTENLOGINTSIA(USERID BIGINT,LOGGEDIN NVARCHAR(10),cnt_internalId NVARCHAR(10),Login_datetime NVARCHAR(10))
	CREATE NONCLUSTERED INDEX IX1 ON #TMPATTENLOGINTSIA(USERID,cnt_internalId,Login_datetime)
	...
	IF OBJECT_ID('tempdb..#TMPSHOPSUBMITACTTSIA') IS NOT NULL
		DROP TABLE #TMPSHOPSUBMITACTTSIA
	CREATE TABLE #TMPSHOPSUBMITACTTSIA(USER_ID BIGINT,cnt_internalId NVARCHAR(10),Shop_Id VARCHAR(100),NEWSHOP_VISITED INT,RE_VISITED INT,VISITED_TIME NVARCHAR(20),SPENT_DURATION VARCHAR(100),DISTANCE_TRAVELLED DECIMAL(18,2))
	CREATE NONCLUSTERED INDEX IX1 ON #TMPSHOPSUBMITACTTSIA(USER_ID,cnt_internalId,Shop_Id)

	SET @Strsql=''
	SET @Strsql='INSERT INTO #TMPSHOPSUBMITACTTSIA(USER_ID,cnt_internalId,Shop_Id,NEWSHOP_VISITED,RE_VISITED,VISITED_TIME,SPENT_DURATION,DISTANCE_TRAVELLED) '
	...
	IF @IsActivateNewOrderScreenwithSize=1
		BEGIN
			IF OBJECT_ID('tempdb..#TMPORDATTRIBUTETSIA') IS NOT NULL
				DROP TABLE #TMPORDATTRIBUTETSIA
			CREATE TABLE #TMPORDATTRIBUTETSIA(USER_ID BIGINT,SHOP_CODE NVARCHAR(200),ORDER_DATE DATETIME,Ordervalue DECIMAL(18,2))
			CREATE NONCLUSTERED INDEX IX1 ON #TMPORDATTRIBUTETSIA (USER_ID ASC)

			SET @Strsql=''
			SET @Strsql='INSERT INTO #TMPORDATTRIBUTETSIA(USER_ID,SHOP_CODE,ORDER_DATE,Ordervalue) '
	...
	IF OBJECT_ID('tempdb..#TMPSHOPVISITREVISITCONVRATETSIA') IS NOT NULL
		DROP TABLE #TMPSHOPVISITREVISITCONVRATETSIA
	CREATE TABLE #TMPSHOPVISITREVISITCONVRATETSIA(USER_ID BIGINT,cnt_internalId NVARCHAR(10),NEWSHOPVISITCONVRATE INT,NORDSHPCNT INT,REVISITCONVRATE INT,RORDSHPCNT INT)
	CREATE NONCLUSTERED INDEX IX1 ON #TMPSHOPVISITREVISITCONVRATETSIA(USER_ID,cnt_internalId)
	
	SET @Strsql=''
	SET @Strsql='INSERT INTO #TMPSHOPVISITREVISITCONVRATETSIA(USER_ID,cnt_internalId,NEWSHOPVISITCONVRATE,NORDSHPCNT,REVISITCONVRATE,RORDSHPCNT) '
	...
	SET @Strsql+='INNER JOIN #TEMPCONTACT CNT ON CNT.cnt_internalId=USR.user_contactId '
	IF @IsActivateNewOrderScreenwithSize=1
		SET @Strsql+='LEFT OUTER JOIN #TMPORDATTRIBUTETSIA ORDH ON SHOPACT.User_Id=ORDH.USER_ID AND SHOPACT.Shop_Id=ORDH.SHOP_CODE '
	ELSE IF @IsActivateNewOrderScreenwithSize=0
	...
	IF OBJECT_ID('tempdb..#TMPSHOPNOVISITTSIA') IS NOT NULL
		DROP TABLE #TMPSHOPNOVISITTSIA
	CREATE TABLE #TMPSHOPNOVISITTSIA(USER_ID BIGINT,CNTVISITTIME INT)
	CREATE NONCLUSTERED INDEX IX1 ON #TMPSHOPNOVISITTSIA(USER_ID)
	
	SET @Strsql=''
	SET @Strsql='INSERT INTO #TMPSHOPNOVISITTSIA(USER_ID,CNTVISITTIME) '
	...
	IF OBJECT_ID('tempdb..#TMPVISITTOCALLTSIA') IS NOT NULL
		DROP TABLE #TMPVISITTOCALLTSIA
	CREATE TABLE #TMPVISITTOCALLTSIA(USERID BIGINT,CNT_INTERNALID NVARCHAR(10),VISITDIFF INT)
	CREATE NONCLUSTERED INDEX IX1 ON #TMPVISITTOCALLTSIA(USERID,CNT_INTERNALID)

	SET @Strsql=''
	SET @Strsql='INSERT INTO #TMPVISITTOCALLTSIA(USERID,CNT_INTERNALID,VISITDIFF) '
	...
	IF OBJECT_ID('tempdb..#TMPATTENWRKHRSTSIA') IS NOT NULL
		DROP TABLE #TMPATTENWRKHRSTSIA
	CREATE TABLE #TMPATTENWRKHRSTSIA(USERID BIGINT,CNT_INTERNALID NVARCHAR(10),LOGINOUTDIFF INT,CNTATTENDANCE INT,WORKDIFF INT,TOTNOOFFULLDAYS INT,TOTNOOFHALFDAYS INT,LESSTWOHRSWORK INT)
	CREATE NONCLUSTERED INDEX IX1 ON #TMPATTENWRKHRSTSIA(USERID,CNT_INTERNALID)

	SET @Strsql=''
	SET @Strsql='INSERT INTO #TMPATTENWRKHRSTSIA(USERID,CNT_INTERNALID,LOGINOUTDIFF,CNTATTENDANCE,WORKDIFF,TOTNOOFFULLDAYS,TOTNOOFHALFDAYS,LESSTWOHRSWORK) '
	...
	IF OBJECT_ID('tempdb..#TMPABSENTTSIA') IS NOT NULL
		DROP TABLE #TMPABSENTTSIA
	CREATE TABLE #TMPABSENTTSIA(USER_ID BIGINT,CNTNOWORKTIME INT)
	CREATE NONCLUSTERED INDEX IX1 ON #TMPABSENTTSIA(USER_ID)

	SET @Strsql=''
	SET @Strsql='INSERT INTO #TMPABSENTTSIA(USER_ID,CNTNOWORKTIME) '
	...
	IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID=OBJECT_ID(N'FTSTERRITORYSALESINCHARGEPERFORMANCEANALY_REPORT') AND TYPE IN (N'U'))
		BEGIN
			CREATE TABLE FTSTERRITORYSALESINCHARGEPERFORMANCEANALY_REPORT
			(
			  USERID INT,
			  SEQ INT,
			  ...
			  TOTALORDVALWITHOUTVISIT DECIMAL(38,2),
			...
			CREATE NONCLUSTERED INDEX IX1 ON FTSTERRITORYSALESINCHARGEPERFORMANCEANALY_REPORT (SEQ)
		END
	DELETE FROM FTSTERRITORYSALESINCHARGEPERFORMANCEANALY_REPORT WHERE USERID=@USERID	

	SET @Strsql=''
	SET @Strsql='INSERT INTO FTSTERRITORYSALESINCHARGEPERFORMANCEANALY_REPORT(USERID,SEQ,EMPCODE,EMPUSERID,EMPID,EMPNAME,CITYID,EMPCITY,STATEID,STATE,BRANCHDESC,DEG_ID,DESIGNATION,DATEOFJOINING,CONTACTNO,'
	SET @Strsql+='REPORTTO,RPTTODESG,ORDSHPCNT,TOTALORDERVALUE,TOTALORDVALWITHOUTVISIT,TARGETORDVALUE,AVGORDERVALUE,SHOPCNT,AVGREVPERSTORE,APPROVEEXPAMT,AVGREVPERVISIT,TOTAL_VISIT,NEWSHOP_VISITED,RE_VISITED,'
	SET @Strsql+='VISITCONVRATE,NEWVISITCONVRATE,REVISITCONVRATE,TOTALVISITTIME,AVGVISITTIME,NOVISITDAYS,AVGTIMETOFSTCALL,AVGWORKDURATION,TOTALKMTRAVELLED,AVGKMTRAVELLED,TOTNOOFFULLDAYS,TOTNOOFHALFDAYS,'
	SET @Strsql+='TOTLEAVEDAYS,TOTALABSENT) '
	...
	SET @Strsql+='DATEOFJOINING,CONTACTNO,REPORTTO,RPTTODESG,ORDSHPCNT,TOTALORDERVALUE,TOTALORDVALWITHOUTVISIT,TARGETORDVALUE,AVGORDERVALUE,SHOPCNT,AVGREVPERSTORE,APPROVEEXPAMT,AVGREVPERVISIT,TOTAL_VISIT,NEWSHOP_VISITED,RE_VISITED,'
	...
	SET @Strsql+='SUM(TOTALORDERVALUE) AS TOTALORDERVALUE,SUM(DISTINCT TOTALORDVALWITHOUTVISIT)-SUM(TOTALORDERVALUE) AS TOTALORDVALWITHOUTVISIT,SUM(DISTINCT TARGETORDVALUE) AS TARGETORDVALUE,CASE WHEN SUM(ORDSHPCNT)<> 0 THEN '
	SET @Strsql+='SUM(TOTALORDERVALUE)/SUM(ORDSHPCNT) ELSE 0.00 END AVGORDERVALUE,SUM(DISTINCT SHOPCNT) AS SHOPCNT,CASE WHEN SUM(DISTINCT SHOPCNT)<> 0 THEN SUM(TOTALORDERVALUE)/SUM(DISTINCT SHOPCNT) ELSE 0.00 END AVGREVPERSTORE,'
	...
	SET @Strsql+='ISNULL(VTC.VISITDIFF,0) AS VISITDIFF,ISNULL(AWD.LOGINOUTDIFF,0) AS LOGINOUTDIFF,ISNULL(AWD.CNTATTENDANCE,0) AS CNTATTENDANCE,ISNULL(DISTANCE_TRAVELLED,0) AS DISTANCE_TRAVELLED,'
	SET @Strsql+='ISNULL(AWD.TOTNOOFFULLDAYS,0) AS TOTNOOFFULLDAYS,ISNULL(AWD.TOTNOOFHALFDAYS,0) AS TOTNOOFHALFDAYS,ISNULL(USRLEAVE.CNTSTATUS,0) AS CNTSTATUS,'
	SET @Strsql+='ISNULL(ABSNT.CNTNOWORKTIME,0)+ISNULL(AWD.LESSTWOHRSWORK,0)+ISNULL(USRRPLEAVE.CNTRPSTATUS,0) AS TOTALABSENT '
	...
	SET @Strsql+='SELECT USERID,LOGGEDIN,cnt_internalId,Login_datetime FROM #TMPATTENLOGINTSIA '
	...
	SET @Strsql+='SELECT USER_ID,cnt_internalId,Shop_Id,NEWSHOP_VISITED,RE_VISITED,VISITED_TIME,SPENT_DURATION,DISTANCE_TRAVELLED FROM #TMPSHOPSUBMITACTTSIA ) AA '
	...
	SET @Strsql+='SELECT USER_ID,cnt_internalId,NEWSHOPVISITCONVRATE,NORDSHPCNT,REVISITCONVRATE,RORDSHPCNT FROM #TMPSHOPVISITREVISITCONVRATETSIA ) AA '
	...
	SET @Strsql+='LEFT OUTER JOIN #TMPSHOPNOVISITTSIA SNV ON USR.USER_ID=SNV.USER_ID '
	SET @Strsql+='LEFT OUTER JOIN #TMPVISITTOCALLTSIA VTC ON USR.USER_ID=VTC.USERID AND CNT.cnt_internalId=VTC.CNT_INTERNALID '
	SET @Strsql+='LEFT OUTER JOIN #TMPATTENWRKHRSTSIA AWD ON USR.USER_ID=AWD.USERID AND CNT.cnt_internalId=AWD.CNT_INTERNALID '
	...
	SET @Strsql+='LEFT OUTER JOIN #TMPABSENTTSIA ABSNT ON USR.USER_ID=ABSNT.USER_ID '
