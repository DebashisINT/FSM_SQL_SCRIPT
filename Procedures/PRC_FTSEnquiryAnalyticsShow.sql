IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSEnquiryAnalyticsShow]') AND type in (N'P', N'PC')) 
BEGIN 
	EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSEnquiryAnalyticsShow] AS'  
END 
GO

--exec PRC_FTSEnquiryAnalyticsShow @USERID='378',@ENQUIRIESFROM='Indiamart,IndiaMart (ARCHER),MccoyMart,Website,Direct Call,Exhibition,Twak,Other',
--		@FROMDATE='2023-11-19',@TODATE='2023-11-23'

ALTER PROCEDURE [dbo].[PRC_FTSEnquiryAnalyticsShow]
(
...
AS
/****************************************************************************************************************************************************************************
Written by Sanchita on 23-11-2023 for V2.0.43		A new report required as Enquiry Analytics. Refer: 27021
****************************************************************************************************************************************************************************/
BEGIN
	SET NOCOUNT ON
	...
	BEGIN
		IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID=OBJECT_ID(N'ENQUIRYANALYTICS_LISTING') AND TYPE IN (N'U'))
			BEGIN
				CREATE TABLE ENQUIRYANALYTICS_LISTING
				(
				  USERID INT,
				  SEQ INT,
				...
				CREATE NONCLUSTERED INDEX IX1 ON ENQUIRYANALYTICS_LISTING (SEQ)
			END
		DELETE FROM ENQUIRYANALYTICS_LISTING WHERE USERID=@USERID
		...
		BEGIN
			...
			SELECT cnt_internalId,cnt_firstName,cnt_middleName,cnt_lastName,cnt_UCC,cnt_contactType FROM TBL_MASTER_CONTACT WHERE cnt_contactType IN('EM')
 			...
 			SET @Strsql=' INSERT INTO ENQUIRYANALYTICS_LISTING(USERID,SEQ,CRM_ID,DATE,CUSTOMER_NAME,CONTACT_PERSON,PHONENO,EMAIL,LOCATION,PRODUCT_REQUIRED,QTY,UOM,ORDER_VALUE,ENQ_DETAILS,CREATED_DATE,'
	...
	begin
		...
		IF EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID=OBJECT_ID(N'ENQUIRYANALYTICS_LISTING') AND TYPE IN (N'U'))
		BEGIN
			SET @TotalPendingEnquiry = (SELECT COUNT(0) FROM ENQUIRYANALYTICS_LISTING WHERE USERID=@USERID AND [STATUS]= 'Pending')
			SET @TotalInProcessEnquiry = (SELECT COUNT(0) FROM ENQUIRYANALYTICS_LISTING WHERE USERID=@USERID AND [ACTIVITY_STATUS]= 'In Process')
			SET @TotalNotInterestedEnquiry = (SELECT COUNT(0) FROM ENQUIRYANALYTICS_LISTING WHERE USERID=@USERID AND [ACTIVITY_STATUS]= 'Not Interested')
			SET @TotalAssignedEnquiry = (SELECT COUNT(0) FROM ENQUIRYANALYTICS_LISTING WHERE USERID=@USERID AND [STATUS]= 'Assigned')
			SET @TotalReassignedEnquiry = (SELECT COUNT(0) FROM ENQUIRYANALYTICS_LISTING WHERE USERID=@USERID AND  (ReASSIGNED_TO is not null  and ReASSIGNED_TO != '') )
			SET @TotalHighRiskEnquiry = (SELECT COUNT(0) FROM ENQUIRYANALYTICS_LISTING WHERE USERID=@USERID AND (ACTIVITY_DATE is null OR ACTIVITY_DATE = '') )
		END
