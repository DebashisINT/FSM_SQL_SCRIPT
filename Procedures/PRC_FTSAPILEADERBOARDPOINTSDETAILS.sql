--EXEC PRC_FTSAPILEADERBOARDPOINTSDETAILS 'OWN',54958,'2','128','M','http://localhost:16126/CommonFolder/ProfileImages/'
...
BEGIN
	...
	IF @ACTION='OVERALL'
		BEGIN
			...
			SET @StrSql+='FROM FTSLEADERBOARDPOINTSDETAILS_REPORT WHERE ACTIVITYMODE='''+@ACTIVITYMODE+''' AND BRANCH_ID='+TRIM(STR(@BRANCHWISE))+' '
	...
	IF @ACTION='OWN'
		BEGIN
			...
			SET @StrSql+='ORDERPOINTS AS [order],ACTIVITIESPOINTS AS activities,POSITION AS position,TOTALSCORES AS totalscore,'
			SET @StrSql+='CASE WHEN PROFILE_PICTURES_URL<>'''' THEN '''+@PROFILEIMAGEPATH+'''+PROFILE_PICTURES_URL ELSE '''' END AS profile_pictures_url '
			SET @StrSql+='FROM FTSLEADERBOARDPOINTSDETAILS_REPORT WHERE USERID='+TRIM(STR(@USERID))+' AND ACTIVITYMODE='''+@ACTIVITYMODE+''' AND BRANCH_ID='+TRIM(STR(@BRANCHWISE))+' '
			IF @ACTIVITYBASED<>'All'
				BEGIN
					IF @ACTIVITYBASED='1'
						SET @StrSql+='AND ATTENDANCEID='''+@ACTIVITYBASED+''' '
