--EXEC PRC_FTSDSSUMMARY_REPORT '2024-01-01','2024-06-30','','','1',378
...
11.0	v2.0.47		Debashis	03/06/2024		A new coloumn shall be added in the below mentioned reports.Refer: 0027402
12.0	v2.0.47		Debashis	10/06/2024		Add a new column at the end named as “Total CDM Days" in selected date range.Refer: 0027496
****************************************************************************************************************************************************************************/
BEGIN
	SET NOCOUNT ON
	...
	--End of Rev 11.0
			...
			--Rev 12.0 && A new column has been added as WORKACTIVITYDESCRIPTION NVARCHAR(500)
			CREATE TABLE #TMPDAYSTARTENDDSS
			(USERID BIGINT,DAYSTTIME NVARCHAR(10),DAYENDTIME NVARCHAR(10),STARTENDDATE NVARCHAR(10),SALE_VALUE NUMERIC(18,2),WORKACTIVITYDESCRIPTION NVARCHAR(500))
			CREATE NONCLUSTERED INDEX IX1 ON #TMPDAYSTARTENDDSS(USERID,STARTENDDATE)

			--Rev 8.0 && A new cloumn has been added as SALE_VALUE
			--Rev 12.0 && A new column has been added as WORKACTIVITYDESCRIPTION
			SET @SqlStr=''
			SET @SqlStr='INSERT INTO #TMPDAYSTARTENDDSS(USERID,DAYSTTIME,DAYENDTIME,STARTENDDATE,SALE_VALUE,WORKACTIVITYDESCRIPTION) '
			...
			SET @SqlStr+='0.00 AS SALE_VALUE,WORKACTIVITYDESCRIPTION '
			...
			SET @SqlStr+='GROUP BY DAYSTEND.User_Id,DAYSTEND.STARTENDDATE,WORKACTIVITYDESCRIPTION '
			SET @SqlStr+='UNION ALL '
			...
			SET @SqlStr+='SUM(DAYSTEND.SALE_VALUE) AS SALE_VALUE,WORKACTIVITYDESCRIPTION '
			SET @SqlStr+='FROM FSMUSERWISEDAYSTARTEND DAYSTEND WITH (NOLOCK) '
			...
			--Rev 11.0
			IF OBJECT_ID('tempdb..#TMPORDERDSS') IS NOT NULL
				DROP TABLE #TMPORDERDSS
			CREATE TABLE #TMPORDERDSS
			(USERID BIGINT,ORDER_DATE NVARCHAR(10),ORDER_VALUE DECIMAL (18,2))
			CREATE NONCLUSTERED INDEX IX1 ON #TMPORDERDSS(USERID,ORDER_DATE)

			SET @SqlStr=''
			SET @SqlStr='INSERT INTO #TMPORDERDSS(USERID,ORDER_DATE,ORDER_VALUE) '
	...
	IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID=OBJECT_ID(N'FTSDSSUMMARY_REPORT') AND TYPE IN (N'U'))
		BEGIN
			...
			--Rev 8.0 && A new cloumn has been added as SALE_VALUE
			--Rev 11.0 && A new cloumn has been added as ORDER_VALUE
			--Rev 12.0 && A new column has been added as TOTALCDMDAYS INT
	...
	--Rev 8.0 && A new cloumn has been added as SALE_VALUE
	--Rev 11.0 && A new cloumn has been added as ORDER_VALUE
	--Rev 12.0 && A new column has been added as TOTALCDMDAYS
	IF @ISPAGELOAD='1'
		BEGIN
	--End of Rev 11.0
			SET @SqlStr=''
			...
			SET @SqlStr+='HREPORTTOID,HREPORTTOUID,HREPORTTO,HRPTTODESG,DATERANGE,OUTLETSMAPPED,NEWSHOP_VISITED,RE_VISITED,TOTAL_VISIT,SALE_VALUE,ORDER_VALUE,DISTANCE_TRAVELLED,AVGTIMESPENTINMARKET,AVGSPENTDURATION,'
			...
			SET @SqlStr+='OUTLETSMAPPED,NEWSHOP_VISITED,RE_VISITED,TOTAL_VISIT,SALE_VALUE,ORDER_VALUE,DISTANCE_TRAVELLED,'
			...
			SET @SqlStr+='SUM(ORDER_VALUE) AS ORDER_VALUE,SUM(DISTANCE_TRAVELLED) AS DISTANCE_TRAVELLED,'
			--End of Rev 6.0
			...
			SET @SqlStr+='DAYSTARTEND.SALE_VALUE,ORD.ORDER_VALUE,ISNULL(DISTANCE_TRAVELLED,0) AS DISTANCE_TRAVELLED,DAYSTARTEND.DAYSTTIME,DAYSTARTEND.DAYENDTIME,'
			...
			SET @SqlStr+='SPENT_DURATION,CNT.cnt_sex AS OUTLETEMPSEX,CNT.GENDERDESC,ISNULL(DAYSTARTEND.TOTALCDMDAYS,0) AS TOTALCDMDAYS '
			...
			--End of Rev 3.0
			SET @SqlStr+='LEFT OUTER JOIN ('
			SET @SqlStr+='SELECT USERID,MIN(DAYSTTIME) AS DAYSTTIME,MAX(DAYENDTIME) AS DAYENDTIME,STARTENDDATE,SUM(SALE_VALUE) AS SALE_VALUE,SUM(ISNULL(TOTALCDMDAYS,0)) AS TOTALCDMDAYS FROM('
			--Rev 4.0
			--SET @SqlStr+='SELECT DAYSTEND.User_Id AS USERID,MIN(CONVERT(VARCHAR(5),CAST(DAYSTEND.STARTENDDATE AS TIME),108)) AS DAYSTTIME,NULL AS DAYENDTIME,CONVERT(NVARCHAR(10),DAYSTEND.STARTENDDATE,105) AS STARTENDDATE '
			...
			--SET @SqlStr+='AND CONVERT(NVARCHAR(10),DAYSTEND.STARTENDDATE,120) BETWEEN CONVERT(NVARCHAR(10),'''+@FROMDATE+''',120) AND CONVERT(NVARCHAR(10),'''+@TODATE+''',120) '
			--SET @SqlStr+='GROUP BY DAYSTEND.User_Id,DAYSTEND.STARTENDDATE '
			SET @SqlStr+='SELECT USERID,DAYSTTIME,DAYENDTIME,STARTENDDATE,SALE_VALUE,CASE WHEN WORKACTIVITYDESCRIPTION=''CDM Day'' THEN 1 ELSE 0 END AS TOTALCDMDAYS FROM #TMPDAYSTARTENDDSS '
			--End of Rev 4.0
			...
			SET @SqlStr+='SELECT USERID,ORDER_DATE,ORDER_VALUE FROM #TMPORDERDSS '
