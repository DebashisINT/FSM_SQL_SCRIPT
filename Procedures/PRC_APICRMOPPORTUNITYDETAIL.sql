		...
		BEGIN
			BEGIN TRAN
				BEGIN TRY
					...
					IF EXISTS(SELECT MP.sProducts_ID FROM Master_sProducts MP
					INNER JOIN @JsonXML.nodes('/root/data')AS TEMPTABLE(XMLproduct) ON MP.sProducts_ID=XMLproduct.value('(product_id/text())[1]','NVARCHAR(100)'))
						BEGIN
					...
					ELSE
						BEGIN
							INSERT INTO FSMCRMOPPORTUNITYDETAIL(DETID,USERID,OPPORTUNITY_ID,SHOP_ID,PRODUCT_ID,PRODUCT_NAME)
							SELECT (@LASTRECORD+1),@USER_ID,@OPPORTUNITY_ID,@SHOP_ID,0 AS PRODUCT_ID,'' AS PRODUCT_NAME
							WHERE EXISTS(SELECT OPPORTUNITY_ID FROM FSMCRMOPPORTUNITYHEADER WHERE OPPORTUNITY_ID=@OPPORTUNITY_ID AND USERID=@USER_ID)
						END

					SELECT OPPORTUNITY_ID FROM FSMCRMOPPORTUNITYHEADER WHERE OPPORTUNITY_ID=@OPPORTUNITY_ID AND USERID=@USER_ID
				COMMIT TRAN
			END TRY
			BEGIN CATCH
				ROLLBACK TRAN
		...
		BEGIN
			BEGIN TRAN
				...
				IF EXISTS(SELECT OPPORTUNITY_ID FROM FSMCRMOPPORTUNITYHEADER WHERE OPPORTUNITY_ID=@OPPORTUNITY_ID AND USERID=@USER_ID)
					BEGIN
						SET @LASTRECORD=(SELECT ISNULL(ID,0) AS ID FROM FSMCRMOPPORTUNITYHEADER WHERE OPPORTUNITY_ID=@OPPORTUNITY_ID AND USERID=@USER_ID)

						DELETE FROM FSMCRMOPPORTUNITYDETAIL WHERE OPPORTUNITY_ID=@OPPORTUNITY_ID AND USERID=@USER_ID
						DELETE FROM FSMCRMOPPORTUNITYHEADER WHERE OPPORTUNITY_ID=@OPPORTUNITY_ID AND USERID=@USER_ID

						INSERT INTO FSMCRMOPPORTUNITYHEADER(ID,USERID,OPPORTUNITY_ID,OPPORTUNITY_DESCRIPTION,SHOP_ID,SHOP_NAME,SHOP_TYPE,OPPORTUNITY_AMOUNT,OPPORTUNITY_STATUS_ID,OPPORTUNITY_STATUS_NAME,
						CREATED_BY,CREATED_DATE,CREATED_TIME,CREATED_DATE_TIME,CREATED_ON,MODIFIED_BY,MODIFIED_DATE_TIME,MODIFIED_ON)
						SELECT (@LASTRECORD),@USER_ID,@OPPORTUNITY_ID,@OPPORTUNITY_DESCRIPTION,@SHOP_ID,@SHOP_NAME,@SHOP_TYPE,@OPPORTUNITY_AMOUNT,@OPPORTUNITY_STATUS_ID,@OPPORTUNITY_STATUS_NAME,
						@USER_ID,@CREATED_DATE,@CREATED_TIME,@CREATED_DATE_TIME,GETDATE(),@USER_ID,@EDIT_DATE_TIME,GETDATE()
						WHERE EXISTS(SELECT SHOP_CODE FROM tbl_Master_shop WHERE Shop_Code=@SHOP_ID)
						...
						ELSE
							BEGIN
								INSERT INTO FSMCRMOPPORTUNITYDETAIL(DETID,USERID,OPPORTUNITY_ID,SHOP_ID,PRODUCT_ID,PRODUCT_NAME)
								SELECT (@LASTRECORD),@USER_ID,@OPPORTUNITY_ID,@SHOP_ID,0 AS PRODUCT_ID,'' AS PRODUCT_NAME
								WHERE EXISTS(SELECT OPPORTUNITY_ID FROM FSMCRMOPPORTUNITYHEADER WHERE OPPORTUNITY_ID=@OPPORTUNITY_ID AND USERID=@USER_ID)
							END
