--EXEC PRC_FTSDASHBOARD_REPORT_HIERARCHY '2019-05-20','5,8,9,14,15,19,26,28,29,38','','','ALL','Summary',378
--EXEC PRC_FTSDASHBOARD_REPORT_HIERARCHY '2018-12-10','','','','EMP','Detail',378
--EXEC PRC_FTSDASHBOARD_REPORT_HIERARCHY '2018-10-26','','','','ALL','Summary',378
--EXEC PRC_FTSDASHBOARD_REPORT_HIERARCHY '2018-12-28','','','','AT_WORK','Detail',378
--EXEC PRC_FTSDASHBOARD_REPORT_HIERARCHY '2018-12-28','','','','AT_WORK','Summary',378
--EXEC PRC_FTSDASHBOARD_REPORT_HIERARCHY '2018-12-28','15','','EMP0000002','AT_WORKTRAVEL','Summary',378
--EXEC PRC_FTSDASHBOARD_REPORT_HIERARCHY '2018-12-07','','','','ON_LEAVE','Detail',378
--EXEC PRC_FTSDASHBOARD_REPORT_HIERARCHY '2018-12-21','15','','','NOT_LOGIN','Detail',378
--EXEC PRC_FTSDASHBOARD_REPORT_HIERARCHY '2018-12-18','','','','GRAPH','Detail',378
-- exec PRC_FTSDASHBOARD_REPORT_HIERARCHY @TODAYDATE='2022-03-30', @STATEID='15,3,19,28', @DESIGNID='',@USERID=378,@EMPID='',@BRANCHID='1,118,119,122',@ACTION='AT_WORK',@RPTTYPE='Detail'

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSDASHBOARD_REPORT_HIERARCHY]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSDASHBOARD_REPORT_HIERARCHY] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSDASHBOARD_REPORT_HIERARCHY]
(
...
15.0	V2.0.36		Sanchita	10-11-2022		When an Employee is mapped with Multiple States and Multiple Branch, the count of that employee should be considered as One
												Refer: 25441
16.0	V2.0.37		Sanchita	01-12-2022		FSM Dashboard : NEW tab shall be implemented "Team Visit - Hierarchy & Channel Wise" after the 'Team Visit' Tab
												Refer: 25468
												NEW SP WRITTEN
17.0	v2.0.38		Sanchita	04-01-2023		Dashboard showing wrong data when clicked on Employee Strength in Visit tab. refer: 25567
18.0	V2.0.40		Sanchita	05-04-2023		ITC user dashboard is taking too much time to respond for AE & AM users login credentials.
												Hierarchy tab optimized. Refer: 25781
****************************************************************************************************************************************************************************/
BEGIN
	SET NOCOUNT ON
	...
	-- End of Rev 12.0
	-- Rev 16.0
	declare @LOGINCNTID NVARCHAR(100), @List_Channel nvarchar(max)
	-- End of Rev 16.0
	...
	set @List_Channel = (SELECT STRING_AGG(EP_CH_ID, ', ') AS List_Channel FROM Employee_ChannelMap where EP_EMP_CONTACTID=@LOGINCNTID GROUP BY EP_EMP_CONTACTID)
	-- End of Rev 16.0
	...
	--	BEGIN
			...
			ELSE
				BEGIN
					...
					SET @Strsql+=' SELECT cnt.emp_cntId,desg.deg_designation,MAX(emp_id) as emp_id,desg.deg_id FROM tbl_trans_employeeCTC AS cnt WITH (NOLOCK) '
					SET @Strsql+=' LEFT OUTER JOIN tbl_master_designation desg WITH (NOLOCK) ON desg.deg_id=cnt.emp_Designation '
					SET @Strsql+=' WHERE cnt.emp_effectiveuntil IS NULL AND desg.deg_designation IN(''DS'',''TL'') '
					SET @Strsql+=' GROUP BY emp_cntId,desg.deg_designation,desg.deg_id '
					SET @Strsql+=' ) DESG ON DESG.emp_cntId=EMP.emp_contactId '
					SET @Strsql+=' WHERE CNT.cnt_contactType IN(''EM'') '
					SET @Strsql+=' AND EXISTS (SELECT LC.EP_CH_ID FROM Employee_ChannelMap LC WITH (NOLOCK) WHERE EP_CH_ID in ('+@List_Channel+') and CNT.cnt_internalId=LC.EP_EMP_CONTACTID)  '
	...
	-- Rev 18.0
	CREATE TABLE #TMPEmployeeBranchMap
		(
			Emp_Contactid NVARCHAR(200) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
			BranchId BIGINT 
		)
	CREATE NONCLUSTERED INDEX IX_Contactid ON #TMPEmployeeBranchMap(Emp_Contactid ASC)

	INSERT INTO #TMPEmployeeBranchMap
	SELECT Emp_Contactid, BranchId FROM FTS_EmployeeBranchMap BMAP WITH (NOLOCK)
	INNER JOIN #EMPHR_EDIT ON BMAP.Emp_Contactid=EMPCODE
	-- End of Rev 18.0

	IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID=OBJECT_ID(N'FTSDASHBOARD_REPORT_HIERARCHY') AND TYPE IN (N'U'))
		BEGIN
			CREATE TABLE FTSDASHBOARD_REPORT_HIERARCHY
			(
			  USERID INT,
			  ACTION NVARCHAR(20),
			  RPTTYPE NVARCHAR(20),
			...
			CREATE NONCLUSTERED INDEX IX1 ON FTSDASHBOARD_REPORT_HIERARCHY (SEQ)
		END
	DELETE FROM FTSDASHBOARD_REPORT_HIERARCHY WHERE USERID=@USERID

	SET @Strsql=''

	-- Rev 18.0 [ In the full query below all FTS_EmployeeBranchMap have been replaced by #TMPEmployeeBranchMap
	IF @ACTION='ALL' AND @RPTTYPE='Summary'
		BEGIN
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,EMPCNT) '
			-- Rev 15.0
			...
			if @ActivateEmployeeBranchHierarchy=0
				SET @Strsql+='INNER JOIN #TMPEmployeeBranchMap BMAP ON CNT.cnt_internalId=BMAP.Emp_Contactid '
			-- End of Rev 13.0
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,AT_WORK) '
			-- Rev 15.0
			...
			SET @Strsql+='SELECT '+STR(@USERID)+' AS USERID,''AT_WORK'' AS ACTION,''Summary'' AS RPTTYPE,CASE WHEN COUNT(cnt_internalId) IS NULL THEN 0 ELSE COUNT(cnt_internalId) END AS AT_WORK FROM ( '
			SET @Strsql+='SELECT '+STR(@USERID)+' AS USERID,''AT_WORK'' AS ACTION,''Summary'' AS RPTTYPE,CNT.cnt_internalId as cnt_internalId '
			-- End of Rev 15.0
			...
			-- Rev 13.0
			if @ActivateEmployeeBranchHierarchy=0
				SET @Strsql+='INNER JOIN #TMPEmployeeBranchMap BMAP WITH (NOLOCK) ON CNT.cnt_internalId=BMAP.Emp_Contactid '
			-- End of Rev 13.0
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,ON_LEAVE) '
			-- Rev 15.0
			...
			SET @Strsql+='SELECT '+STR(@USERID)+' AS USERID,''ON_LEAVE'' AS ACTION,''Summary'' AS RPTTYPE,CASE WHEN COUNT(cnt_internalId) IS NULL THEN 0 ELSE COUNT(cnt_internalId) END AS ON_LEAVE FROM ( '
			SET @Strsql+='SELECT '+STR(@USERID)+' AS USERID,''ON_LEAVE'' AS ACTION,''Summary'' AS RPTTYPE,CNT.cnt_internalId as cnt_internalId '
			-- End of Rev 15.0
			SET @Strsql+='FROM tbl_master_employee EMP WITH (NOLOCK) '
			SET @Strsql+='INNER JOIN #TEMPCONTACT CNT ON CNT.cnt_internalId=EMP.emp_contactId '
			-- Rev 13.0
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,NOT_LOGIN) '
			...
			--SET @Strsql=''
			--SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,NOT_LOGIN) '
			--SET @Strsql+='SELECT '+STR(@USERID)+' AS USERID,''NOT_LOGIN'' AS ACTION,''Summary'' AS RPTTYPE,SUM(ISNULL(EMPCNT,0))-(SUM(ISNULL(AT_WORK,0))+SUM(ISNULL(ON_LEAVE,0))) AS NOT_LOGIN FROM FTSDASHBOARD_REPORT_HIERARCHY '
	...
	ELSE IF @ACTION='AT_WORK' AND @RPTTYPE='Summary'
		BEGIN
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,STATEID,STATE,DESIGNATION,EMPCNT) '
	...
	ELSE IF @ACTION='EMP' AND @RPTTYPE='Detail'
		BEGIN
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCHID,BRANCH,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,CONTACTNO,REPORTTOUID,REPORTTO,DEPARTMENT) '
			...
			begin
				...
				SET @Strsql+='INNER JOIN #TMPEmployeeBranchMap BMAP WITH (NOLOCK) ON EMP.emp_contactId=BMAP.Emp_Contactid '
	...
	ELSE IF @ACTION='AT_WORK' AND @RPTTYPE='Detail'
		BEGIN
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCHID,BRANCH,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,CONTACTNO,REPORTTOUID,REPORTTO,LOGGEDIN,LOGEDOUT,'
	...
	ELSE IF @ACTION='AT_WORKTRAVEL' AND @RPTTYPE='Summary'
		BEGIN
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCHID,BRANCH,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,CONTACTNO,REPORTTOUID,REPORTTO,DISTANCE_COVERED) '
			...
			begin
				-- Rev 13.0
				--SET @Strsql+='INNER JOIN (select ROW_NUMBER() OVER (PARTITION BY BM.Emp_Contactid ORDER BY BM.Emp_Contactid,B.BRANCH_ID) row_num, B.BRANCH_ID, B.BRANCH_DESCRIPTION, BM.Emp_Contactid from tbl_master_branch B INNER JOIN #TMPEmployeeBranchMap BM on B.BRANCH_ID=BM.BranchId ) BR on BR.Emp_Contactid=EMP.emp_contactId and row_num=1 '
			...
			begin
				...
				BEGIN
					-- Rev 13.0
					--SET @Strsql+=' and EXISTS (SELECT Branch_Id from #BRANCHID_LIST AS BR inner join #TMPEmployeeBranchMap BMAP on BR.Branch_Id=BMAP.BranchId and BMAP.Emp_Contactid=DB.EMPCODE ) '
					SET @Strsql+=' AND EXISTS (SELECT Branch_Id from #BRANCHID_LIST AS BR WHERE BR.Branch_Id=DB.BranchId) '
					-- End of Rev 13.0
				END
			end
	...
	ELSE IF @ACTION='ON_LEAVE' AND @RPTTYPE='Detail'
		BEGIN
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCHID,BRANCH,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,REPORTTOUID,REPORTTO,LEAVEDATE,DEPARTMENT) '
	...
	ELSE IF @ACTION='GRAPH' AND @RPTTYPE='Detail'
		BEGIN
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCHID,BRANCH,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,CONTACTNO,REPORTTOUID,REPORTTO) '
			SET @Strsql+='SELECT '+STR(@USERID)+' AS USERID,''EMP'' AS ACTION,''Detail'' AS RPTTYPE,ROW_NUMBER() OVER(ORDER BY DESIGNATION) AS SEQ,BRANCHID,BRANCH,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,'
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCHID,BRANCH,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,REPORTTOUID,REPORTTO,LOGGEDIN,LOGEDOUT,'
			...
			--Rev 14.0 && WITH (NOLOCK) has been added in all tables
			SET @Strsql=''
			SET @Strsql='INSERT INTO FTSDASHBOARD_REPORT_HIERARCHY(USERID,ACTION,RPTTYPE,SEQ,BRANCHID,BRANCH,EMPID,EMPCODE,EMPNAME,STATEID,STATE,DEG_ID,DESIGNATION,REPORTTOUID,REPORTTO,LEAVEDATE) '
			SET @Strsql+='SELECT '+STR(@USERID)+' AS USERID,''ON_LEAVE'' AS ACTION,''Detail'' AS RPTTYPE,ROW_NUMBER() OVER(ORDER BY DESIGNATION) AS SEQ,BRANCHID,BRANCH,EMPID,EMPCODE,EMPNAME,STATEID,STATE,'
	...
	DROP TABLE #TEMPSHOPUSER
	-- Rev 18.0
	DROP TABLE #TMPEmployeeBranchMap
	-- End of Rev 18.0
