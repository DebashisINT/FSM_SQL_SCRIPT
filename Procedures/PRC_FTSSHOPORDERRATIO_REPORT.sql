--EXEC PRC_FTSSHOPORDERRATIO_REPORT '2018-04-01','2018-12-12',378

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSSHOPORDERRATIO_REPORT]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSSHOPORDERRATIO_REPORT] AS'
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSSHOPORDERRATIO_REPORT]
(
...
BEGIN
	...
	SET @Strsql=''
	SET @Strsql='SELECT SEQ,Shop_Code,ORDERCNT,Shop_Name,CONVERT(DECIMAL(18,2),ORDVALUE) AS ORDVALUE,CONVERT(DECIMAL(18,2),(ORDVALUE/ORDERCNT)) AS RATIO FROM ('
	SET @Strsql+='SELECT ROW_NUMBER() OVER(ORDER BY SHOP.Shop_Name) AS SEQ,'
	SET @Strsql+='SHOP.Shop_Code,COUNT(ORDHEAD.OrderCode) AS ORDERCNT,SHOP.Shop_Name,SUM(ORDDET.ORDVALUE) AS ORDVALUE '
	...
	SET @Strsql+='GROUP BY SHOP.Shop_Code,SHOP.Shop_Name '
