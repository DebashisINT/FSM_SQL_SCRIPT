...
-- EXEC PRC_Send_FTSSORDERSUMMARYMTD_AUTOMIL
-- EXEC PRC_FTSORDERSUMMARYMTD_REPORT_AUTOMIL @MONTH='JUL', @YEARS='2023', @USERID=378


IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_Send_FTSSORDERSUMMARYMTD_AUTOMIL]') AND type in (N'P', N'PC')) 
BEGIN 
	EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_Send_FTSSORDERSUMMARYMTD_AUTOMIL] AS'  
END 
GO

ALTER PROCEDURE [dbo].[PRC_Send_FTSSORDERSUMMARYMTD_AUTOMIL]
AS
/***************************************************************************************************************************************************
...
BEGIN
	...
	select EMAILID, USER_ID from Honeywell.[dbo].TBL_ORDERSUMMARYMTDAutomail
	open db_cursor_mail
	...
	begin
		...
		--set @sqlMailQry = 'EXEC Honeywell.[dbo].PRC_FTSORDERSUMMARYMTD_REPORT_AUTOMIL @MONTH='''+@CurrentMonth+''',@YEARS='''+@CurrentYear+''',@USERID=378 '
		set @sqlMailQry = 'EXEC Honeywell.[dbo].PRC_FTSORDERSUMMARYMTD_REPORT_AUTOMIL @MONTH='''+@CurrentMonth+''',@YEARS='''+@CurrentYear+''',@USERID='+cast(@user_id as varchar(50))+' '
		EXEC SP_EXECUTESQL @sqlMailQry

		if (select count(0) from Honeywell.[dbo].ORDERSUMMARYMTD_MTD_MAIL)>0
		begin
			set @SubjectText = 'ORDER SUMMARY(MTD) REPORT AS ON '+ @ReportToDate
			set @bodyText = 'Please find attached excel file for ORDER SUMMARY(MTD) REPORT AS ON '+ @ReportToDate
			set @filename = 'ORDER SUMMARY MTD REPORT_'+ @ReportToDate+'.csv'

			set @sqlQry = 'SET NOCOUNT ON select * from Honeywell..ORDERSUMMARYMTD_MTD_MAIL '
			--set @sqlQry = 'select 1 '
		...
		IF EXISTS (SELECT * FROM sys.objects WHERE object_id=OBJECT_ID(N'ORDERSUMMARYMTD_MTD_MAIL') AND TYPE IN (N'U'))
			DROP TABLE Honeywell..ORDERSUMMARYMTD_MTD_MAIL
