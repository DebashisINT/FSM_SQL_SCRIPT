...
-- exec PRC_SendTeamVisitReportAUtomail_ALL
-- EXEC PRC_FTSTEAMVISITATTENDANCE_REPORT_AUTOMAIL '2022-10-05','2022-10-10','1,118,120,121,122,123,124,125,126,127,128,129,130,','','',378,1
-- File attachment or query results size exceeds allowable value of 1000000 bytes.


IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_SendTeamVisitReportAUtomail_ALL]') AND type in (N'P', N'PC')) 
BEGIN 
	EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_SendTeamVisitReportAUtomail_ALL] AS'  
END 
GO

ALTER PROCEDURE [dbo].[PRC_SendTeamVisitReportAUtomail_ALL]
AS
/***************************************************************************************************************************************************
...
BEGIN
	...
	SELECT STRING_AGG(Branch_Id, ', ') WITHIN GROUP (ORDER BY Branch_Id ) AS Branch_Id, 'ALLBRANCH',EMAILID 
			FROM FSM_ITC..TBL_VisitDetailsAutomail_ALL TV where EMAILID<>'' GROUP BY EMAILID
	open db_cursor_mail
