...
BEGIN
	...
	(SELECT COUNT(0) FROM tbl_trans_shopActivitysubmit WHERE Shop_Id=shop.Shop_Code) + (SELECT COUNT(0) FROM tbl_trans_shopActivitysubmit_Archive WHERE Shop_Id=shop.Shop_Code) AS total_visit_count,
	(SELECT ISNULL(ISNULL(MAX(visited_time),shop.Lastvisit_date),GETDATE()) FROM (
	SELECT visited_time,Shop_Id FROM tbl_trans_shopActivitysubmit
	UNION ALL
	SELECT visited_time,Shop_Id FROM tbl_trans_shopActivitysubmit_Archive
	...
	INNER JOIN tbl_shoptype typs ON typs.shop_typeId=shop.type
	LEFT OUTER JOIN FTS_ShopMoreDetails DTLS ON DTLS.SHOP_ID=shop.Shop_ID
	LEFT OUTER JOIN tbl_trans_shopActivitysubmit SHOPACT ON SHOP.Shop_Code=SHOPACT.Shop_Id 
