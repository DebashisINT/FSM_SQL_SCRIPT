IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[View_ALLUserhiarchy]') AND type in (N'V')) 
BEGIN 
	EXEC dbo.sp_executesql @statement = N'CREATE VIEW [View_ALLUserhiarchy] AS SELECT 1 A' 
END 
GO 

ALTER VIEW [dbo].[View_ALLUserhiarchy]
--WITH ENCRYPTION
AS
...
left join  (SELECT cnt.emp_cntId,desg.deg_designation,MAX(emp_id) as emp_id,desg.deg_id,emp_reportTo FROM tbl_trans_employeeCTC AS cnt  
...
left join tbl_master_employee as emp1 on emp1.emp_id=empctc.emp_reportTo
left join tbl_master_user as usr1 on  usr1.user_contactId=emp1.emp_contactId 
