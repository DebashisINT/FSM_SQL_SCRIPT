...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSHORIZONTALEMPLOYEEPERFORMANCESUMDET_FETCH_AUTOMIL]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSHORIZONTALEMPLOYEEPERFORMANCESUMDET_FETCH_AUTOMIL] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSHORIZONTALEMPLOYEEPERFORMANCESUMDET_FETCH_AUTOMIL]
(
...
BEGIN
	...
	IF @REPORTTYPE='Summary'
		BEGIN
			...
			SET @Strsql+='- CAST(CAST(ISNULL(CAST((DATEPART(HOUR,ISNULL(ATTEN.LOGGEDIN,''00:00:00'')) * 60) AS FLOAT) +CAST(DATEPART(MINUTE,ISNULL(ATTEN.LOGGEDIN,''00:00:00'')) * 1 AS FLOAT),0) AS VARCHAR(100)) AS FLOAT) AS WORKINGDURATION,'
			-- End of Rev 5.0
