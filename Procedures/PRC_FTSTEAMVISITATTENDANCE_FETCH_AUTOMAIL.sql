--EXEC PRC_FTSTEAMVISITATTENDANCE_FETCH_AUTOMAIL '2022-02-20','2022-02-28','','EMS0000812','',378
--EXEC PRC_FTSTEAMVISITATTENDANCE_FETCH_AUTOMAIL '2022-01-01','2022-10-10','','','1,4',378

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSTEAMVISITATTENDANCE_FETCH_AUTOMAIL]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSTEAMVISITATTENDANCE_FETCH_AUTOMAIL] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSTEAMVISITATTENDANCE_FETCH_AUTOMAIL]
(
...
BEGIN
	...
	SET @SqlStr+='SELECT USERID,MIN(LOGGEDIN) AS LOGGEDIN,MAX(LOGEDOUT) AS LOGEDOUT FROM('
	SET @SqlStr+='SELECT USERID,LOGGEDIN,LOGEDOUT FROM #TMPATTENLOGINOUTTV '
	SET @SqlStr+=') INOUT GROUP BY USERID) ATTENINOUT ON USR.user_id=ATTENINOUT.USERID '
	--End of Rev 4.0
