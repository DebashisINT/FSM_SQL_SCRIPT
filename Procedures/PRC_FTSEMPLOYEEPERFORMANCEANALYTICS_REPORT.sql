...
BEGIN
	...
	IsDistributorwiseNearbyShopVisit NVARCHAR(10))
	CREATE NONCLUSTERED INDEX IX1 ON #TMPATTENLOGINOUT(USERID,cnt_internalId,Login_datetime)

	SET @Strsql=''
	SET @Strsql='INSERT INTO #TMPATTENLOGINOUT(USERID,LOGGEDIN,LOGEDOUT,cnt_internalId,Login_datetime,ATTEN_STATUS,IsDistributorwiseNearbyShopVisit) '
	...
	SET @Strsql+='CASE WHEN Isonleave=''false'' THEN ''At Work'' ELSE ''On Leave'' END AS ATTEN_STATUS,ATTEN.IsDistributorwiseNearbyShopVisit FROM tbl_fts_UserAttendanceLoginlogout ATTEN '
	...
	SET @Strsql+='GROUP BY ATTEN.User_Id,CNT.cnt_internalId,CONVERT(NVARCHAR(10),ATTEN.Work_datetime,105),ATTEN.Isonleave,ATTEN.IsDistributorwiseNearbyShopVisit '
	...
	SET @Strsql+='SELECT '+STR(@USERID)+' AS USERID,ROW_NUMBER() OVER(ORDER BY LOGIN_DATETIME) AS SEQ,LOGIN_DATETIME,LOGGEDIN,LOGEDOUT,VISITED_TIME,CONTACTNO,STATEID,STATE,BRANCHDESC,'
	...
	SET @Strsql+='USR.user_loginId AS CONTACTNO,RPTTO.REPORTTO,RPTTO.RPTTODESG,HRPTTO.HREPORTTO,HRPTTO.HRPTTODESG,LOGIN_DATETIME,ATTEN.LOGGEDIN AS LOGGEDIN,ATTEN.LOGEDOUT AS LOGEDOUT,SHOPACT.VISITED_TIME,'
	...
	SET @Strsql+='SHOP.Address+'' ''+SHOP.Shop_Owner_Contact AS SHOPADDR_CONTACT,SHOP.CITY_NAME AS SHOP_DISTRICT,SHOP.Pincode AS SHOP_PINCODE,SHOPDD.Shop_Code AS DD_CODE,'
	...
	SET @Strsql+='INNER JOIN ('
	SET @Strsql+='SELECT USERID,MIN(LOGGEDIN) AS LOGGEDIN,MAX(LOGEDOUT) AS LOGEDOUT,cnt_internalId,Login_datetime,ATTEN_STATUS,MDD_YESNO FROM('
	SET @Strsql+='SELECT USERID,LOGGEDIN,LOGEDOUT,cnt_internalId,Login_datetime,ATTEN_STATUS,IsDistributorwiseNearbyShopVisit AS MDD_YESNO FROM #TMPATTENLOGINOUT '
	SET @Strsql+=') LOGINLOGOUT GROUP BY USERID,cnt_internalId,Login_datetime,ATTEN_STATUS,MDD_YESNO '
	...
	SET @Strsql+='LEFT OUTER JOIN( '
	SET @Strsql+='SELECT DISTINCT A.Shop_CreateUser,A.assigned_to_dd_id,A.Shop_Code,A.Shop_Name,A.Address,A.Shop_Owner_Contact,Type,PS.PARTYSTATUS AS SHOPDDSTATUS FROM tbl_Master_shop A '
	...
	SET @Strsql+='USR.user_loginId AS CONTACTNO,RPTTO.REPORTTO,RPTTO.RPTTODESG,HRPTTO.HREPORTTO,HRPTTO.HRPTTODESG,LOGIN_DATETIME,ATTEN.LOGGEDIN AS LOGGEDIN,ATTEN.LOGEDOUT AS LOGEDOUT,NULL AS VISITED_TIME,'
