...
BEGIN
	...
	IF @ACTION='ALL'
	BEGIN
			INSERT INTO PRODUCTSPECIALPRICELIST(SEQ ,USERID ,SlNo,SPECIALPRICEID ,BRANCH_ID ,BRANCH ,PRODUCT_ID ,PRODUCTCODE ,PRODUCTNAME ,SPECIALPRICE,Employee_Name,EMPINTERNALID )

		
			SELECT ROW_NUMBER() OVER (ORDER BY ID DESC),@USERID,ROW_NUMBER() OVER (ORDER BY ID DESC) SlNo,ID,MB.BRANCH_ID,branch_description,sProducts_ID,sProducts_Code PRODUCT_CODE,sProducts_Name,SPECIAL_PRICE 
			--Rev 1.0 
			...
			INNER JOIN Master_sProducts  MP ON MP.sProducts_ID=PSPB.PRODUCT_ID
