...
--EXEC PRC_FTSTEAMVISITATTENDANCESUMMARY_FETCH '2022-02-20','2022-10-10','','','1,4',378
...
3.0		v2.0.33		Debashis	10/10/2022		Team Visit: report the following field value:
...
BEGIN
	...
	IF OBJECT_ID('tempdb..#TMPATTENLOGINOUTTVS') IS NOT NULL
		DROP TABLE #TMPATTENLOGINOUTTVS
	CREATE TABLE #TMPATTENLOGINOUTTVS
	(USERID BIGINT,LOGGEDIN NVARCHAR(10),LOGEDOUT NVARCHAR(10))
	CREATE NONCLUSTERED INDEX IX1 ON #TMPATTENLOGINOUTTVS(USERID,LOGGEDIN)

	SET @SqlStr=''
	SET @SqlStr='INSERT INTO #TMPATTENLOGINOUTTVS(USERID,LOGGEDIN,LOGEDOUT) '
	SET @SqlStr+='SELECT ATTEN.User_Id AS USERID,CONVERT(VARCHAR(5),CAST(ATTEN.Login_datetime AS TIME),108) AS LOGGEDIN,NULL AS LOGEDOUT '
	...
	SET @SqlStr+='UNION ALL '
	SET @SqlStr+='SELECT ATTEN.User_Id AS USERID,NULL AS LOGGEDIN,CONVERT(VARCHAR(5),CAST(ATTEN.Logout_datetime AS TIME),108) AS LOGEDOUT '
	SET @SqlStr+='FROM tbl_fts_UserAttendanceLoginlogout ATTEN WITH (NOLOCK) '
	SET @SqlStr+='INNER JOIN tbl_master_user USR WITH (NOLOCK) ON USR.user_id=ATTEN.User_Id AND USR.user_inactive=''N'' '
	SET @SqlStr+='INNER JOIN #TEMPCONTACT CNT ON CNT.cnt_internalId=USR.user_contactId '
	...
	SET @SqlStr+='CASE WHEN ISNULL(ATTENINOUT.LOGGEDIN,'''')<>'''' THEN  ''Present'' ELSE '
	SET @SqlStr+='CASE WHEN (SELECT FORMAT(CAST('''+@FROMDATE+''' AS DATE), ''dddd''))=''Sunday'' THEN ''Weekly Off'' ELSE ''Not Logged In'' END END AS ATTEN_STATUS,'
	--End of Rev 3.0
	...
	SET @SqlStr+='SELECT USERID,(LOGGEDIN) AS LOGGEDIN,(LOGEDOUT) AS LOGEDOUT FROM('
	SET @SqlStr+='SELECT USERID,LOGGEDIN,LOGEDOUT FROM #TMPATTENLOGINOUTTVS '
	SET @SqlStr+=') INOUT) ATTENINOUT ON USR.user_id=ATTENINOUT.USERID '
	--End of Rev 3.0
