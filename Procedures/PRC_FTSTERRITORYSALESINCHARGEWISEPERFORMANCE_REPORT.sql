...
BEGIN
	...
	IF @IsActivateNewOrderScreenwithSize=1
		BEGIN
			...
			SET @Strsql+='GROUP BY ORDH.USER_ID,ORDH.SHOP_ID,ORDH.ORDER_DATE '
	...
	SET @Strsql+='INNER JOIN #TEMPCONTACT CNT ON CNT.cnt_internalId=USR.user_contactId '
	IF @IsActivateNewOrderScreenwithSize=1
		SET @Strsql+='INNER JOIN #TMPORDATTRIBUTETSI ORDH ON SHOPACT.User_Id=ORDH.USER_ID AND SHOPACT.Shop_Id=ORDH.SHOP_CODE '
	ELSE IF @IsActivateNewOrderScreenwithSize=0
		SET @Strsql+='INNER JOIN tbl_trans_fts_Orderupdate ORDH ON SHOPACT.User_Id=ORDH.userID AND SHOPACT.Shop_Id=ORDH.Shop_Code '
	SET @Strsql+='WHERE CONVERT(NVARCHAR(10),SHOPACT.visited_time,120) BETWEEN CONVERT(NVARCHAR(10),'''+@FROMDATE+''',120) AND CONVERT(NVARCHAR(10),'''+@TODATE+''',120) '
	SET @Strsql+='AND SHOPACT.Is_Newshopadd=1 '
	...
	SET @Strsql+='SUM(TOTALORDERVALUE) AS TOTALORDERVALUE,SUM(TARGETORDVALUE) AS TARGETORDVALUE,CASE WHEN SUM(ORDSHPCNT)<> 0 THEN SUM(TOTALORDERVALUE)/SUM(ORDSHPCNT) ELSE 0.00 END AVGORDERVALUE,'
	...
	SET @Strsql+='CASE WHEN SUM(APPROVEEXPAMT)<> 0 THEN SUM(TOTALORDERVALUE)/SUM(APPROVEEXPAMT) ELSE 0.00 END APPROVEEXPAMT,'
