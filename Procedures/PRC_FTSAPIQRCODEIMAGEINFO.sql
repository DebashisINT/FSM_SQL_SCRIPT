		...
		BEGIN
			IF NOT EXISTS(SELECT USERID FROM FSMQRCODEIMAGEINFO WHERE USERID=@USER_ID)
				BEGIN
					...
					SELECT @BaseURL+ISNULL(QRCODEIMAGEPATH,'') AS qr_img_link FROM FSMQRCODEIMAGEINFO WHERE USERID=@USER_ID AND QRCODEIMAGEPATH=@PathImage
				END
		END
	ELSE IF @ACTION='FETCHQRCODEIMAGE'
		BEGIN
			SELECT @BaseURL+ISNULL(QRCODEIMAGEPATH,'') AS qr_img_link FROM FSMQRCODEIMAGEINFO WHERE USERID=@USER_ID
		END
