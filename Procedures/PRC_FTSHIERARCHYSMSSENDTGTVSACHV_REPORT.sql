--EXEC PRC_FTSHIERARCHYSMSSENDTGTVSACHV_REPORT '','378'

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSHIERARCHYSMSSENDTGTVSACHV_REPORT]') AND TYPE in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSHIERARCHYSMSSENDTGTVSACHV_REPORT] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSHIERARCHYSMSSENDTGTVSACHV_REPORT]
(
...
Module	   : Hierarchy SMS for Target Vs Achievement
...
BEGIN
	...
	SET @Strsql=''
	--Rev 1.0
	--SET @Strsql='SELECT ISNULL(STUFF((SELECT '' | '' +EMPNAME+'' ''+TGTNEWCOUNTER+'', ''+CASE WHEN ACHVNEWSHOPVISITED<>'''' THEN ACHVNEWSHOPVISITED ELSE ''NC ACHV - 0'' END +'' | ''+'
	--SET @Strsql+='TGTREVISIT+'', ''+CASE WHEN ACHVREVISITSHOP<>'''' THEN ACHVREVISITSHOP ELSE ''RV ACHV - 0'' END FROM( '
	--SET @Strsql+='SELECT EMPNAME,''NC TGT - ''+LTRIM(RTRIM(STR(TGT.NEWCOUNTER/TGT.DAYS))) AS TGTNEWCOUNTER,''RV TGT - ''+LTRIM(RTRIM(STR(TGT.REVISIT/DAYS))) AS TGTREVISIT,'
	--SET @Strsql+=''''' AS ACHVNEWSHOPVISITED,'''' AS ACHVREVISITSHOP FROM('
	SET @Strsql='SELECT ISNULL(STUFF((SELECT '' | '' +EMPNAME+'': ''+CASE WHEN SUM(TGTNEWCOUNTER)<>0 THEN ''NC TGT - ''+LTRIM(RTRIM(STR(SUM(TGTNEWCOUNTER)))) ELSE ''NC TGT - 0'' END +'', ''+'
	...
	SET @Strsql+='CASE WHEN SUM(ACHVREVISITSHOP)<>0 THEN ''RV ACHV - ''+LTRIM(RTRIM(STR(SUM(ACHVREVISITSHOP)))) ELSE ''RV ACHV - 0'' END FROM( '
	SET @Strsql+='SELECT EMPNAME,(TGT.NEWCOUNTER/TGT.DAYS) AS TGTNEWCOUNTER,(TGT.REVISIT/TGT.DAYS) AS TGTREVISIT,0 AS ACHVNEWSHOPVISITED,0 AS ACHVREVISITSHOP FROM('
	--End of Rev 1.0
	...
	--End of Rev 1.0
	SET @Strsql+=') AS TGT WHERE EXISTS (SELECT USER_ID from #USER_LIST AS USL WHERE USL.user_id=TGT.REPORTTOUSER) '
	IF @STATEID<>''
		SET @Strsql+='AND EXISTS (SELECT State_Id from #STATEID_LIST AS ST WHERE ST.State_Id=TGT.STATEID) '
	SET @Strsql+='UNION ALL '
	--Rev 1.0
	--SET @Strsql+='SELECT EMPNAME,'''' AS TGTNEWCOUNTER,'''' AS TGTREVISIT,''NC ACHV - ''+LTRIM(RTRIM(STR(ISNULL(ACHV.NEWSHOPVISITED,0)))) AS ACHVNEWSHOPVISITED,''RV ACHV - ''+LTRIM(RTRIM(STR(ACHV.REVISITSHOP))) AS ACHVREVISITSHOP FROM('
	SET @Strsql+='SELECT EMPNAME,0 AS TGTNEWCOUNTER,0 AS TGTREVISIT,ISNULL(ACHV.NEWSHOPVISITED,0) AS ACHVNEWSHOPVISITED,ISNULL(ACHV.REVISITSHOP,0) AS ACHVREVISITSHOP FROM('
	--End of Rev 1.0
	SET @Strsql+='SELECT USR.user_id,CNT.cnt_internalId AS EMPCODE,ISNULL(CNT.CNT_FIRSTNAME,'''')+'' ''+ISNULL(CNT.CNT_MIDDLENAME,'''')+'' ''+ISNULL(CNT.CNT_LASTNAME,'''') AS EMPNAME,ST.ID AS STATEID,'
	SET @Strsql+='ST.state AS STATE,DESG.DEG_ID,DESG.deg_designation AS DESIGNATION,USR.user_loginId AS CONTACTNO,RPTTO.user_id AS REPORTTOUSER,RPTTO.REPORTTO,'
	...
	SET @Strsql+=') AS ACHV WHERE EXISTS (SELECT USER_ID from #USER_LIST AS USL WHERE USL.user_id=ACHV.REPORTTOUSER) '
	IF @STATEID<>''
		SET @Strsql+='AND EXISTS (SELECT State_Id from #STATEID_LIST AS ST WHERE ST.State_Id=ACHV.STATEID) '
	--Rev 1.0
	--SET @Strsql+=') AS DB FOR XML PATH('''')), 2, 2, ''''),'''') SMS '
	SET @Strsql+=') AS DB GROUP BY EMPNAME FOR XML PATH('''')), 2, 2, ''''),'''') SMS '
	--End of Rev 1.0
	--SELECT @Strsql
