...
9.0			Sanchita	28-12-2022		V2.0.38			Shop import will not happen if there is any duplicate contact number.
														If duplicate contact exist then duplicate shopcode will be generated. 
														Refer: 25558
...
-- Rev 9.0
	...
	begin
		...
		where [Contact] is not NULL and exists (select [Contact] from @IMPORT_TABLE group by [Contact] having count([Contact])>1 and [Contact]=temp.[Contact] )
		) A
		...
		SELECT 
		[State],[City],[Area],[Cluster],[Type],[Shop type],[Entity Type],[Assigned To PP],[DD Type],[Assigned To DD],[Party Name],[Party Code],[Party Status],[Address],[Pin Code],[Owner],[DOB],[Anniversary],
		[Contact],[Alternate Contact],[Alternate Contact 1],[Email],[Alternate Email],[Status],[Entity Category],[Owner PAN],[Owner Aadhaar],[GSTIN],[Trade License],[Location]
		-- Rev 10.0
		--,[Group/Beat],[Account Holder],[Account No],[Bank Name],[IFSC Code],[UPI ID],
