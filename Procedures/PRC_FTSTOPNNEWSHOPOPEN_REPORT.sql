--EXEC PRC_FTSTOPNNEWSHOPOPEN_REPORT '2017-11-24','2018-11-29',378
--EXEC PRC_FTSTOPNNEWSHOPOPEN_REPORT '2018-04-01','2018-12-11',378

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[PRC_FTSTOPNNEWSHOPOPEN_REPORT]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [PRC_FTSTOPNNEWSHOPOPEN_REPORT] AS' 
END
GO

ALTER PROCEDURE [dbo].[PRC_FTSTOPNNEWSHOPOPEN_REPORT]
(
...
Module	   : Top N Salesmen for New Shop Opened
****************************************************************************************************************************************************************************/
BEGIN
	SET NOCOUNT ON
	...
	SET @Strsql+='SHOPVISIT.NEWSHOPVISIT,USR.user_loginId AS CONTACTNO,RPTTO.user_id AS RPTTOUSERID,RPTTO.emp_reportTo AS RPTTOID,'
	...
	SET @Strsql+='SELECT User_Id,SUM(NEWSHOPVISIT) AS NEWSHOPVISIT FROM( '
	SET @Strsql+='SELECT Shop_CreateUser AS User_Id,ISNULL(COUNT(DISTINCT Shop_ID),0) AS NEWSHOPVISIT FROM tbl_Master_shop '
	...
	SET @Strsql+=') AS NEWSHOPVISIT ORDER BY NEWSHOPVISIT DESC '
	--SELECT @Strsql
	EXEC (@Strsql)
